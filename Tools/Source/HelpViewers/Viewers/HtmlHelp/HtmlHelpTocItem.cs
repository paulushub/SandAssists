...
{
	/// <summary>
	/// The class <c>HtmlHelpTocItem</c> implements a toc-entry item
	/// </summary>
	public sealed class HtmlHelpTocItem
	{
		/// <summary>
		...
		/// </summary>
		private List<string> _infoTypeStrings = new List<string>();
		/// <summary>
		...
		/// </summary>
        private List<HtmlHelpTocItem> _children = new List<HtmlHelpTocItem>();
		/// <summary>
		...
		/// Parent item
		/// </summary>
		private HtmlHelpTocItem _parent=null;
		...
		public HtmlHelpTocItem(string name, string local, int ImageIndex, string chmFile)
		{
		...
		public HtmlHelpTocItem(int topicOffset, int ImageIndex, CHMFile associatedFile)
		{
		...
		internal void Dump(ref BinaryWriter writer, bool writeFilename)
		{
			...
			for(int i=0; i<_infoTypeStrings.Count; i++)
				writer.Write(_infoTypeStrings[i]);

			writer.Write(_children.Count);

			for(int i=0; i<_children.Count; i++)
			{
				HtmlHelpTocItem child = _children[i];
		...
		internal void ReadDump(ref BinaryReader reader, bool readFilename)
		{
			...
			for(i=0; i<nCnt; i++)
			{
				HtmlHelpTocItem child = new HtmlHelpTocItem();
		...
		/// <summary>
		/// Gets the list which holds all information types/categories this item is associated
		/// </summary>
		internal IList<string> InfoTypeStrings
		{
		...
		/// </summary>
		/// <remarks>Each entry in the list is of type HtmlHelpTocItem</remarks>
        public IList<HtmlHelpTocItem> Children
		{
			get { return _children; }
			set { _children = (List<HtmlHelpTocItem>)value; }
		}
        ...
        internal FileObject ContentFile
		{
			get
			{
