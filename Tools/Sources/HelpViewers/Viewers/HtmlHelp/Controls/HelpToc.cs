...
using Sandcastle.Viewers.Properties;
...
namespace Sandcastle.Viewers.HtmlHelp.Controls
{
	...
	/// </summary>
	public class HelpToc : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public event EventHandler<TocEventArgs> TocSelected;
		...
		{
			if (this.TocSelected != null)
			{
                this.TocSelected(this, e);
			}
		}
		...
		public void ReloadImageList()
		{
			...
			{
				hhImages.TransparentColor = Color.FromArgb(128, 64, 0);
				hhImages.Images.AddStrip(Resources.MsdnHelp);
			}
			else
			{
				hhImages.TransparentColor = Color.Fuchsia;
				hhImages.Images.AddStrip(Resources.HtmlHelp);
			}
		}
		...
		{
			OnTocSelected( new TocEventArgs((HtmlHelpTocItem)(e.Node.Tag)) );
		}
		...
		/// <param name="filter">information type/category filter</param>
        private void BuildTOC(IList<HtmlHelpTocItem> tocItems, TreeNodeCollection col, InfoTypeCategoryFilter filter)
		{
			foreach( HtmlHelpTocItem curItem in tocItems )
			{
				bool bAdd = true;
