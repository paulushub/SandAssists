...
{
    public sealed class CSharpCompletionBinding : NRefactoryCodeCompletionBinding
	{
        ...
        sealed class CSharpCodeCompletionDataProvider : CodeCompletionDataProvider
		{
        ...
        sealed class PointerArrowCompletionDataProvider : CodeCompletionDataProvider
		{
			protected override ResolveResult Resolve(ExpressionResult expressionResult, int caretLineNumber, int caretColumn, string fileName, string fileContent)
			{
			...
			{
				ICSharpCode.TextEditor.Document.ITextDocument document = textArea.Document;
		...
		ExpressionContext FindExactContextForNewCompletion(CodeEditorControl editor, string documentToCursor,
