...
{
    public sealed class CSharpAdvancedHighlighter : AsynchronousAdvancedHighlighter
	{
...
//				}
		...
		protected override void MarkWords(int lineNumber, LineSegment currentLine, 
            IList<TextWord> words)
		{
            ...
            string lineText = document.GetText(currentLine.Offset, currentLine.Length);
				...
				if (Char.IsLetter(lineText[word.Offset]) || lineText[word.Offset] == '_') 
                {
					...
					if (result.Expression != null) 
                    {
						...
						if (rr is MixedResolveResult || rr is TypeResolveResult) {
							changedLine = true;
                            word.SyntaxColor = document.HighlightingStrategy.GetColorFor("TypeReference");
						} else if (rr == null) {
							changedLine = true;
                            word.SyntaxColor = document.HighlightingStrategy.GetColorFor("UnknownEntity");
						}
					}
				}
			}
