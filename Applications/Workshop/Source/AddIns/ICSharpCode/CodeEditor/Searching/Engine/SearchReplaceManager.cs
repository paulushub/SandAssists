...
{
	public static class SearchReplaceManager
	{
        //public static SearchDialog SearchAndReplaceDialog = null;

        private static Search find = new Search();

        private static SearchResultMatch lastResult = null;

        private static bool foundAtLeastOneItem = false;

        private static SearchSelection textSelection;
		...
		static void MarkResult(List<TextEditorControl> textAreas, SearchResultMatch result)
		{
            ...
            textArea.ActiveTextAreaControl.Caret.Position = document.OffsetToPosition(result.Offset);
            LineSegment segment = document.GetLineSegmentForOffset(result.Offset);

            int lineNr = segment.LineNumber;
            if (!document.BookmarkManager.IsMarked(lineNr, BookmarkType.TextMark))
            {
                document.BookmarkManager.ToggleMarkAt(
                    new TextLocation(result.Offset - segment.Offset, lineNr));
            }
		}
		...
		static void ShowReplaceDoneMessage(int count, IProgressMonitor monitor)
		{
            ...
            else 
            {
                ...
                MessageService.ShowMessage(StringParser.Parse(
                    "${res:ICSharpCode.TextEditor.Document.SearchReplaceManager.ReplaceAllDone}", new string[,] { { "Count", count.ToString() } }),
                                           "${res:Global.FinishedCaptionText}");
                if (monitor != null) 
                    monitor.ShowingDialog = false;
			}
            ...
            IProgressMonitor monitor)
		{
            ...
            textSelection = new SearchSelection(offset, length);
