...
namespace ICSharpCode.XmlEditor
{
	...
	/// </summary>
	public sealed class XmlFormattingStrategy : DefaultFormattingStrategy
	{
		...
		private void TryIndent(TextArea textArea, int begin, int end)
		{
            ...
            settings.XmlResolver = null;  // prevent XmlReader from loading external DTDs
			using (StringReader stringReader = new StringReader(document.TextContent)) {
                XmlReader r = XmlReader.Create(stringReader, settings);
                IXmlLineInfo lineInfo = r as IXmlLineInfo;
				//r.XmlResolver = null;
				while (r.Read()) {
					if (wasEmptyElement) {
					...
					while (lineInfo.LineNumber > nextLine) { // caution: here we compare 1-based and 0-based line numbers
					...
					if (r.NodeType == XmlNodeType.Element) {
						...
						if (lineInfo.LineNumber < begin)
							currentIndentation = GetIndentation(textArea, lineInfo.LineNumber - 1);
						if (r.Name.Length < 16)
