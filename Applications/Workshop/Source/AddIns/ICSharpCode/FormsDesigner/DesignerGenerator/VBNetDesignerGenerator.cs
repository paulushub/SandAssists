...
namespace ICSharpCode.FormsDesigner
{
	public class VBNetDesignerGenerator : AbstractDesignerGenerator
	{
		...
		protected override DomRegion GetReplaceRegion(ICSharpCode.TextEditor.Document.ITextDocument document, IMethod method)
		{
			DomRegion r = method.BodyRegion;
			return new DomRegion(r.BeginLine + 1, 1, r.EndLine, 1);
		}
		
		protected override void RemoveFieldDeclaration(ICSharpCode.TextEditor.Document.ITextDocument document, IField field)
		{
		...
		protected override void ReplaceFieldDeclaration(ICSharpCode.TextEditor.Document.ITextDocument document, IField oldField, string newFieldDeclaration)
		{
			// In VB, the field region begins at the start of the declaration
			// and ends on the first column of the line following the declaration.
