...
namespace ICSharpCode.FormsDesigner
{
	...
	{
		protected override DomRegion GetReplaceRegion(ITextDocument document, IMethod method)
		{
		...
		protected override int GetCursorLine(ITextDocument document, IMethod method)
		{
			if (document == null)
		...
		public static void CreateComponentEvent(
			IClass c, ITextDocument document,
			Type eventType, string eventMethodName, string body, out int lineNumber)
		{
			...
			string tabs = CodeEditorProperties.Instance.IndentationString;
			tabs += tabs;
