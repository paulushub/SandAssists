...
namespace ICSharpCode.FormsDesigner
{
	...
	/// </summary>
	public sealed class DesignerSourceCodeStorage : IEnumerable<KeyValuePair<OpenedFile, ITextDocument>>
	{
		...
		/// <summary>
        /// Gets the <see cref="ITextDocument"/> associated with the specified file or
		/// <c>null</c> if the file is not registered with the current instance.
		/// </summary>
		public ITextDocument this[OpenedFile file] {
			get {
				FileContent c;
		...
		public IEnumerator<KeyValuePair<OpenedFile, ITextDocument>> GetEnumerator()
		{
			foreach (KeyValuePair<OpenedFile, FileContent> entry in this.fileContents) {
				yield return new KeyValuePair<OpenedFile, ITextDocument>(entry.Key, entry.Value.Document);
			}
		}
		...
		/// </summary>
		public void AddFile(OpenedFile file, ITextDocument document, Encoding encoding)
		{
		...
		public void AddFile(OpenedFile file, ITextDocument document, Encoding encoding, bool doNotLoad)
		{
		...
		/// <summary>
        /// Stores the content of a file in an <see cref="ITextDocument"/>
		...
		sealed class FileContent
		{
			...
			public FileContent(ITextDocument document, Encoding encoding)
				: this(document, encoding, false)
			{
			}
			
			public FileContent(ITextDocument document, Encoding encoding, bool doNotLoad)
			{
				if (document == null)
