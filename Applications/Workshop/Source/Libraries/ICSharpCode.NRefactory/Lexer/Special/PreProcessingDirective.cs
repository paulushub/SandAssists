		...
		public static PreprocessingDirective VBToCSharp(PreprocessingDirective dir)
		{
			...
			if (cmd.Equals("#End", StringComparison.OrdinalIgnoreCase)) {
				if (arg.ToLowerInvariant().StartsWith("region")) {
					cmd = "#endregion";
					arg = "";   
				} else if (arg.Equals("if", StringComparison.OrdinalIgnoreCase)) {
					cmd = "#endif";
					arg = "";
				}
			} else if (cmd.Equals("#Region", StringComparison.OrdinalIgnoreCase)) {
				cmd = "#region";
			} else if (cmd.Equals("#If", StringComparison.OrdinalIgnoreCase)) {
				cmd = "#if";
				if (arg.ToLowerInvariant().EndsWith(" then"))
					arg = arg.Substring(0, arg.Length - 5);
			} else if (cmd.Equals("#Else", StringComparison.OrdinalIgnoreCase)) {
				if (dir.Expression != null)
					cmd = "#elif";
				else
					cmd = "#else";
			} else if (cmd.Equals("#ElseIf", StringComparison.OrdinalIgnoreCase)) {
				cmd = "#elif";
			}
