...
namespace ICSharpCode.TextEditor.Document
{
	...
	/// <summary>
	/// The default <see cref="ITextDocument"/> implementation.
	/// </summary>
	public sealed class TextDocument : ITextDocument
	{
		...
		public string TextContent {
			...
			set {
                ...
                OnDocumentChanging(new DocumentEventArgs(this, 0, 0, value));
		...
		public string GetText(ITextSegment segment)
		{
...
//
		...
		public void UpdateSegmentListOnDocumentChange<T>(IList<T> list, 
            DocumentEventArgs e) where T : ITextSegment
		{
			int removedCharacters = e.Length > 0 ? e.Length : 0;
			int insertedCharacters = e.Text != null ? e.Text.Length : 0;
			for (int i = 0; i < list.Count; ++i) {
				ITextSegment s = list[i];
        ...
        public event DocumentEventHandler DocumentChanging;
		public event DocumentEventHandler DocumentChanged;
		
		// UPDATE STUFF
		List<TextAreaUpdate> updateQueue = new List<TextAreaUpdate>();
		
		public IList<TextAreaUpdate> UpdateQueue {
			get {
				return updateQueue;
			}
		}
		...
		[Conditional("DEBUG")]
		internal static void ValidatePosition(ITextDocument document, TextLocation position)
		{
