...
namespace ICSharpCode.TextEditor.Document
{
	...
	{
		public static HighlightingStrategy Parse(SyntaxMode syntaxMode, XmlReader xmlReader)
		{
			return Parse(null, syntaxMode, xmlReader);
		}

		public static HighlightingStrategy Parse(HighlightingStrategy highlighter, SyntaxMode syntaxMode, XmlReader xmlReader)
		{
			if (syntaxMode == null)
			...
			try {
                ...
                settings.Schemas.Add("", XmlReader.Create(shemaStream));
				...
				if (highlighter == null)
					highlighter = new HighlightingStrategy(doc.DocumentElement.Attributes["name"].InnerText);
				
				if (doc.DocumentElement.HasAttribute("extends")) {
					KeyValuePair<SyntaxMode, ISyntaxModeFileProvider> entry = 
					...
					if (entry.Key == null) {
						...
						highlighter = Parse(highlighter, entry.Key, entry.Value.GetSyntaxModeFile(entry.Key, null));
