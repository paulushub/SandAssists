...
{
	public class HighlightingStrategy : IHighlightingStrategyUsingRuleSets
	{
		private string name;
        private List<HighlightRuleSet> rules;
		...
		public IList<HighlightRuleSet> Rules {
			get {
				return rules;
			}
		}
...
//		}
		...
		public HighlightColor GetColor(ITextDocument document, LineSegment currentSegment, int currentOffset, int currentLength)
		{
			return GetColor(defaultRuleSet, document, currentSegment, currentOffset, currentLength);
		}

		protected virtual HighlightColor GetColor(HighlightRuleSet ruleSet, ITextDocument document, LineSegment currentSegment, int currentOffset, int currentLength)
		{
			if (ruleSet != null) {
				if (ruleSet.Reference != null) {
		...
		public virtual void MarkTokens(ITextDocument document)
		{
			if (Rules.Count == 0) {
				return;
			}
		...
		bool MarkTokensInLine(ITextDocument document, int lineNumber, ref bool spanChanged)
		{
			currentLineNumber = lineNumber;
		...
		public virtual void MarkTokens(ITextDocument document, IList<LineSegment> inputLines)
		{
			if (Rules.Count == 0) {
				return;
			}
...
//
		...
		List<TextWord> ParseLine(ITextDocument document)
		{
		...
		protected virtual void OnParsedLine(ITextDocument document, LineSegment currentLine, List<TextWord> words)
		{
		}
		
		protected virtual bool OverrideSpan(string spanBegin, ITextDocument document, List<TextWord> words, Span span, ref int lineOffset)
		{
			return false;
		}
		...
		/// correct color.
		/// </summary>
		void PushCurWord(ITextDocument document, ref HighlightColor markNext, List<TextWord> words)
		{
...
//									document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.SingleLine, document.GetLineNumberForOffset(document.Caret.Offset)));
		...
		/// </summary>
		static string GetRegString(LineSegment lineSegment, char[] expr, int index, ITextDocument document)
		{
			int j = 0;
		...
		/// </summary>
		static bool MatchExpr(LineSegment lineSegment, char[] expr, int index, ITextDocument document, bool ignoreCase)
		{
