...
namespace ICSharpCode.TextEditor
{
	...
	/// </summary>
    public sealed class TextAreaMouseHandler
	{
		TextArea  textArea;
			...
			if (textArea.TextView.DrawingPosition.Contains(mousepos.X, mousepos.Y)) {
				...
				if (button == MouseButtons.Left) {
					...
					if (marker != null && marker.IsFolded) {
						...
						textArea.SelectionManager.SetSelection(new TextSelection(textArea.TextView.Document, startLocation, endLocation));
		...
		int FindNext(ITextDocument document, int offset, char ch)
		{
		...
		int FindWordStart(ITextDocument document, int offset)
		{
			LineSegment line = document.GetLineSegmentForOffset(offset);
		...
		int FindWordEnd(ITextDocument document, int offset)
		{
			LineSegment line   = document.GetLineSegmentForOffset(offset);
