...
namespace ICSharpCode.TextEditor
{
	...
	/// </summary>
    public sealed class GutterMargin : AbstractMargin, IDisposable
	{
        private static Cursor rightLeftCursor;

		private StringFormat numberStringFormat;
		
        public GutterMargin(TextArea textArea)
            : base(textArea)
        {
            numberStringFormat = (StringFormat)StringFormat.GenericTypographic.Clone();
        ...
        {
            if (numberStringFormat != null)
            {
                numberStringFormat.Dispose();
                numberStringFormat = null;
            }
        }
				...
				if((Control.ModifierKeys & Keys.Shift) != 0) {
					if(!textArea.SelectionManager.HasSomethingSelected && realline != textArea.Caret.Position.Y) {
						...
						{ // at or below starting selection, place the cursor on the next line
							...
							{
								textArea.SelectionManager.SetSelection(new TextSelection(textArea.Document, selectionStartPos, new TextLocation(0, realline + 1)));
								textArea.Caret.Position = new TextLocation(0, realline + 1);
							}
							else
							{
								textArea.SelectionManager.SetSelection(new TextSelection(textArea.Document, selectionStartPos, new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1, realline)));
						...
						{ // prior lines to starting selection, place the cursor on the same line as the new selection
							...
							selectionStartPos = textArea.Caret.Position;
							// whole line selection - start of line to start of next line
							textArea.SelectionManager.SetSelection(new TextSelection(textArea.Document, selectionStartPos, new TextLocation(selectionStartPos.X, selectionStartPos.Y)));
					...
					if (realline < textArea.Document.TotalNumberOfLines - 1)
					{
						textArea.SelectionManager.SetSelection(new TextSelection(textArea.Document, selectionStartPos, new TextLocation(selectionStartPos.X, selectionStartPos.Y + 1)));
						textArea.Caret.Position = new TextLocation(selectionStartPos.X, selectionStartPos.Y + 1);
					}
					else
					{
						textArea.SelectionManager.SetSelection(new TextSelection(textArea.Document, new TextLocation(0, realline), new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1, selectionStartPos.Y)));
