...
namespace ICSharpCode.SharpDevelop.Commands
{
	...
	/// </summary>
    public sealed class ChooseLayoutCommand : AbstractComboBoxCommand
	{
		int editIndex  = -1;
		int resetIndex = -1;
        ...
        public ComboBox ComboBox
        {
            get
            {
                ToolBarComboBox toolbarItem = this.Owner as ToolBarComboBox;
                if (toolbarItem != null)
                {
                    return toolbarItem.ComboBox;
                }

                MenuComboBox menuItem = this.Owner as MenuComboBox;
                if (menuItem != null)
                {
                    isMenuOwner = true;
                    return menuItem.ComboBox;
                }
		...
		protected override void OnOwnerChanged(EventArgs e)
		{
            ...
            if (isMenuOwner)
            {
                MenuComboBox menuItem = (MenuComboBox)this.Owner;
                if (menuItem.Alignment != ToolStripItemAlignment.Right)
                {
                    menuItem.Alignment = ToolStripItemAlignment.Right;
                    Padding margin = menuItem.Margin;
                    margin.Right += 6;
