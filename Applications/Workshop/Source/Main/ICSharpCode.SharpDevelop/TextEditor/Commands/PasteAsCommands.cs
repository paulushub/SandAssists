...
namespace ICSharpCode.SharpDevelop.TextEditor.Commands
{
	public abstract class PasteAsCommand : AbstractMenuCommand
	{
		...
		protected string GetIndentation(ICSharpCode.TextEditor.Document.ITextDocument document, int line)
		{
		...
		protected override void Run(TextEditorControl editor, string clipboardText)
		{
			...
			using (StringReader reader = new StringReader(clipboardText)) {
				...
				while ((line = reader.ReadLine()) != null) {
                    ...
                    {
						expression = expression.Operator(BinaryOperatorType.Concat, 
                            ExpressionBuilder.Identifier("Environment").Member("NewLine")).Operator(BinaryOperatorType.Concat, newExpr);
					}
				}
			}
			if (expression == null)
