...
namespace ICSharpCode.SharpDevelop.TextEditor.Commands
{
	...
	public class ClassMemberMenuBuilder : ISubmenuBuilder
	{
		...
		void CreateProperty(object sender, EventArgs e, bool includeSetter)
		{
			...
			codeGen.InsertCodeAfter(member, new RefactorDocument(textEditor.Document),
		...
		void CreateChangedEvent(object sender, EventArgs e)
		{
			...
			TextEditorControl textEditor = FindReferencesAndRenameHelper.JumpBehindDefinition(member);
			member.DeclaringType.ProjectContent.Language.CodeGenerator.CreateChangedEvent(member, new RefactorDocument(textEditor.Document));
			ParserService.ParseCurrentViewContent();
		}
		...
		void FindOverrides(object sender, EventArgs e)
		{
			...
			foreach (IClass derivedClass in derivedClasses) {
				...
				if (m != null && !m.Region.IsEmpty) {
                    ...
                    res.SearchDocumentInfo = FindReferencesAndRenameHelper.GetDocumentInformation(m.DeclaringType.CompilationUnit.FileName);
                    results.Add(res);
				}
			}
