...
{
	/// <summary>
	/// This represents a code error marker. It is needed by the error drawer.
	/// </summary>
	public sealed class CodeErrorMarker : TextMarker
	{
		private Task task;
		
		public CodeErrorMarker(int offset, int length, Task task)
			: base(offset, length, TextMarkerType.WaveLine, 
            (task.TaskType == TaskType.Error) ? Color.Red : Color.Orange)
		{
			this.task = task;
	...
	/// </summary>
	public sealed class CodeErrorDrawer : IDisposable
    {
        ...
        public CodeErrorDrawer(TextEditorControl textEditor)
		{
		...
		void OnRemoved(object sender, TaskEventArgs e)
		{
			...
			foreach (TextMarker marker in textEditor.Document.MarkerStrategy.TextMarker) {
				CodeErrorMarker ve = marker as CodeErrorMarker;
		...
		void AddTask(Task task, bool refresh)
		{
			...
			if (task.Line >= 0 && task.Line < textEditor.Document.TotalNumberOfLines) {
				...
				textEditor.Document.MarkerStrategy.AddMarker(new CodeErrorMarker(offset, length, task));
