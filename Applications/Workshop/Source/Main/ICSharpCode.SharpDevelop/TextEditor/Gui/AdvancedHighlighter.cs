...
namespace ICSharpCode.SharpDevelop.TextEditor.Gui
{
	...
	public interface IAdvancedHighlighter : IDisposable
	{
		...
		void BeginUpdate(ITextDocument document, IList<LineSegment> inputLines);
		void EndUpdate();
		
		/// <summary>
		/// Gives your highlighter the chance to change the highlighting of the words.
		/// </summary>
		void MarkLine(int lineNumber, LineSegment currentLine, IList<TextWord> words);
	}
	...
	public abstract class AsynchronousAdvancedHighlighter : IAdvancedHighlighter
	{
        ...
        private Dictionary<LineSegment, IList<TextWord>> outstanding;
        private TextEditorControl textEditor;
        private ITextDocument document;

        private int immediateMarkLimit;

        private bool markVisibleOnly;
        private int markVisibleAdditional;

        private int directMark;

        protected AsynchronousAdvancedHighlighter()
        {
            lockObject            = new object();
            immediateMarkLimit    = 3;
            markVisibleOnly       = true;
            markVisibleAdditional = 5;

            outstanding = new Dictionary<LineSegment, IList<TextWord>>();
        }

        ~AsynchronousAdvancedHighlighter()
        {
            this.Dispose(false);
        }
        ...
        #region IAdvancedHighlighter Members
        ...
        public virtual void BeginUpdate(ITextDocument document, IList<LineSegment> inputLines)
		{
        ...
        void IAdvancedHighlighter.MarkLine(int lineNumber, LineSegment currentLine,
            IList<TextWord> words)
        {
            if (directMark > 0)
            {
                directMark--;
        ...
        protected abstract void MarkWords(int lineNumber, LineSegment
            currentLine, IList<TextWord> words);
        ...
        protected virtual void MarkOutstanding()
		{
			...
			IEnumerable<KeyValuePair<LineSegment, IList<TextWord>>> oldOutstanding;
			lock (lockObject) {
			...
			foreach (KeyValuePair<LineSegment, IList<TextWord>> pair in oldOutstanding) {
