...
namespace ICSharpCode.SharpDevelop.TextEditor.Gui
{
	public class CtrlSpaceCompletionDataProvider : CodeCompletionDataProvider
	{
		...
		void AddTemplates(TextArea textArea, char charTyped)
		{
            ...
            IList<ICompletionData> templateCompletionData = new TemplateCompletionDataProvider().GenerateCompletionData(fileName, textArea, charTyped);
			if (templateCompletionData == null || templateCompletionData.Count == 0)
				return;
			for (int i = 0; i < completionData.Count; i++) {
				if (completionData[i].ImageIndex == ClassBrowserIconService.KeywordIndex) {
					string text = completionData[i].Text;
					for (int j = 0; j < templateCompletionData.Count; j++) {
			...
			if (suggestedData != null) {
				completionData.Sort(CompletionData.Compare);
