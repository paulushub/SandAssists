...
namespace ICSharpCode.SharpDevelop.TextEditor.Gui
{
	public class TemplateCompletionDataProvider : AbstractCompletionDataProvider
	{
        ...
        public override IList<ICompletionData> GenerateCompletionData(string fileName, TextArea textArea, char charTyped)
		{
			...
			bool automaticInsert = this.AutomaticInsert || CodeEditorProperties.Instance.AutoInsertTemplates;
		...
		class TemplateCompletionData : CompletionData
		{
			...
			public override bool InsertAction(TextArea textArea, char ch)
			{
				if (ch == '\t' || automaticInsert) {
					((CodeEditorControl)textArea.MotherTextEditorControl).InsertTemplate(template);
					return false;
				} else {
