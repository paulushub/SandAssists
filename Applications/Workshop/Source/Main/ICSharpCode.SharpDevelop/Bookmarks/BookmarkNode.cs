...
namespace ICSharpCode.SharpDevelop.Bookmarks
{
	...
	public class BookmarkNode : ExtTreeNode
	{
		...
		public BookmarkNode(BookmarkEx bookmark)
            : this()
		{
			drawDefault = false;
        ...
        protected override int MeasureItemWidth(DrawTreeNodeEventArgs e)
        {
            ...
            {
                x += MeasureTextWidth(g, bookmark.Document.GetText(line).Replace(
                    "\t", "    "), BoldMonospacedFont);
            }
            return x;
        }
        ...
        protected override void DrawForeground(DrawTreeNodeEventArgs e)
        {
            ...
            spaceSize = g.MeasureString("-", RegularBigFont, new PointF(0, 0), 
		...
		{
			if (bookmark.Document != null) {
				line = bookmark.Document.GetLineSegment(
                    Math.Min(bookmark.LineNumber, bookmark.Document.TotalNumberOfLines));
        ...
        private float DrawDocumentWord(Graphics g, string word, PointF position, 
            Font font, Color foreColor)
		{
			...
			g.DrawString(word, font, BrushRegistry.GetBrush(foreColor),
			             position, sf);

			return wordSize.Width;
		}
		
		private void DrawLine(Graphics g, LineSegment line, float yPos, float xPos, 
            TreeNodeStates state)
		{
				...
				foreach (TextWord word in line.Words) 
                {
					switch (word.Type) 
                    {
						...
						case TextWordType.Word:
							xPos += DrawDocumentWord(g, word.Word, new PointF(xPos, yPos),
            ...
            {
				DrawDocumentWord(g, bookmark.Document.GetText(line),
                    new PointF(xPos, yPos), RegularMonospacedFont, 
