...
namespace ICSharpCode.SharpDevelop.Bookmarks
{
	...
	public class BookmarkFolderNode : ExtFolderNode
	{
		...
		public void AddMark(BookmarkEx mark)
		{
			int index = -1;
		...
		public void RemoveMark(BookmarkEx mark)
		{
			marks.Remove(mark);
			if (isInitialized) {
		...
		protected override void Initialize()
		{
			Nodes.Clear();
			if (marks.Count > 0) {
				ITextDocument document = marks[0].Document;
				if (document != null && document.HighlightingStrategy == null) {
					document.HighlightingStrategy = HighlightingStrategyFactory.CreateHighlightingStrategyForFile(fileName);
				}
				foreach (BookmarkEx mark in marks) {
