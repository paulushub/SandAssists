	...
	public sealed class BuildEngine
	{
        ...
        public static void BuildInGui(IBuildable project, BuildOptions options)
		{
			...
			if (guiBuildProgressMonitor != null) 
            {
				...
				results.Add(new BuildError(null, Core.ResourceService.GetString(
                    "MainWindow.CompilerMessages.MSBuildAlreadyRunning")));
		...
		sealed class MessageViewSink : IBuildFeedbackSink
		{
            ...
            public void Done(bool success)
            {
                throw new InvalidOperationException(
                    "The Done(IBuildable,BuildOptions,BuildResults) method should be called instead.");
            }
        ...
        public static void StartBuild(IBuildable project, BuildOptions options, 
            IBuildFeedbackSink realtimeBuildFeedbackSink, IProgressMonitor progressMonitor)
		{
			if (project == null)
		...
		void BuildDone()
		{
			...
			if (!buildIsCancelled) {
				foreach (BuildNode n in nodeDict.Values) {
					if (!n.buildFinished) {
						throw new Exception(
                            "All workers done, but a project did not finish building");
					}
				}
			}
