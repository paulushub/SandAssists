...
namespace ICSharpCode.SharpDevelop.Project
{
	...
	public class Solution : SolutionFolder, IDisposable, IMSBuildEngineProvider, IBuildable
	{
        ...
        private Dictionary<string, ISolutionFolder> guidDictionary;
		
		public Solution()
		{
            fileName = String.Empty;
            guidDictionary = new Dictionary<string, ISolutionFolder>(
                StringComparer.CurrentCultureIgnoreCase);
            buildEngine = MSBuildInternals.CreateEngine();
		...
		static ProjectSection SetupSolutionLoadSolutionProjects(Solution newSolution, 
            StreamReader sr, IProgressMonitor progressMonitor)
		{
			...
			while (true) {
				...
				if (match.Success) {
					...
					if (projectGuid == FolderGuid) {
						...
						IProject newProject = LanguageBindingService.LoadProject(
                            newSolution, location, title, projectGuid, progressMonitor);
		...
		internal List<ProjectConfigurationPlatformMatching>
			GetActiveConfigurationsAndPlatformsForProjects(string solutionConfiguration, 
            string solutionPlatform)
		{
			List<ProjectConfigurationPlatformMatching> results = 
                new List<ProjectConfigurationPlatformMatching>();
			ProjectSection prjSec = GetProjectConfigurationsSection();
			Dictionary<string, SolutionItem> dict = 
                new Dictionary<string, SolutionItem>(StringComparer.OrdinalIgnoreCase);
		...
		public static Solution Load(string fileName)
		{
			...
			if (extension == ".CMBX") {
				...
				Converter.CombineToSolution.ConvertSolution(newSolution, fileName);
            ...
            else if (extension == ".PRJX") 
            {
				...
				newSolution.fileName = Path.ChangeExtension(fileName, ".sln");
				Converter.CombineToSolution.ConvertProject(newSolution, fileName);
				if (newSolution.FixSolutionConfiguration(newSolution.Projects)) {
					newSolution.Save();
				}
			} 
