...
namespace ICSharpCode.SharpDevelop.Project.Converter
{
	...
	public abstract class LanguageConverter : AbstractMenuCommand
	{
		...
		protected virtual void FixExtensionOfExtraProperties(FileProjectItem item, 
            string sourceExtension, string targetExtension)
		{
            string metadataTest = "Include";
			
			List<KeyValuePair<string, string>> replacements = 
                new List<KeyValuePair<string, string>>();
			foreach (string metadataName in item.MetadataNames) 
            {
				if (metadataTest.Equals(metadataName, StringComparison.OrdinalIgnoreCase))
					continue;
	...
	public abstract class NRefactoryLanguageConverter : LanguageConverter
	{
		...
		protected override void CopyProperties(IProject sourceProject, IProject targetProject)
		{
			...
			// WarningsAsErrors/NoWarn refers to warning numbers that are completely 
            // different for the two compilers.
