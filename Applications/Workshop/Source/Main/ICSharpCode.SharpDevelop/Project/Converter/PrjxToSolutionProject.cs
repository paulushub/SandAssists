	...
	public static class PrjxToSolutionProject
	{
		public class Conversion
		{
			...
			public string Negate(string booleanString)
			{
				...
				return booleanString.Equals("false", StringComparison.OrdinalIgnoreCase).ToString();
			}
		...
		public static void RunConverter(TextReader inFile, string outFile, string script, Conversion conversion)
		{
			...
			using (XmlReader reader = XmlReader.Create(inFile)) {
				//Create an XmlWriter which outputs to memory.
            ...
            using (XmlWriter writer = XmlWriter.Create(outFile, writeSettings))
            {
                XmlReaderSettings readSettings = new XmlReaderSettings();
                readSettings.IgnoreWhitespace = false;
				using (XmlReader reader = XmlReader.Create(
                    new StringReader(stringWriter.ToString()), readSettings)) {
