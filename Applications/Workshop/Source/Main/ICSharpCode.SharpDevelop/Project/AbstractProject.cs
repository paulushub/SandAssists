...
namespace ICSharpCode.SharpDevelop.Project
{
	...
	public abstract class AbstractProject : AbstractSolutionFolder, IProject
	{
		...
		public virtual Properties CreateMemento()
		{
            ...
            properties.Set("bookmarks", 
                BookmarkExManager.GetProjectBookmarks(this).ToArray());
		...
		public virtual void SetMemento(Properties memento)
		{
			...
			foreach (BookmarkEx mark in memento.Get("bookmarks", new BookmarkEx[0])) 
            {
                BookmarkExManager.AddMark(mark);
			}
		...
		public FileProjectItem FindFile(string fileName)
		{
			lock (SyncRoot) {
				if (findFileCache == null) {
					findFileCache = new Dictionary<string, FileProjectItem>(
