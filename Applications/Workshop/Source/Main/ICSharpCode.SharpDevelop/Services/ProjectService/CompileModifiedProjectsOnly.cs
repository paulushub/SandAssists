...
namespace ICSharpCode.SharpDevelop.Project
{
	...
	static class BuildModifiedProjectsOnlyService
	{
        ...
        static void BuildEngine_GuiBuildFinished(object sender, BuildEventArgs e)
		{
            ...
            if (e.Options.ProjectTarget == BuildTarget.Clean || 
                e.Options.TargetForDependencies == BuildTarget.Clean)
            {
                lock (unmodifiedProjects)
                {
			...
			if (ProjectService.OpenSolution != null) {
				foreach (IProject p in ProjectService.OpenSolution.Projects) {
                    if (p.FindFile(e.FileName) != null || 
                        FileUtility.IsEqualFileName(p.FileName, e.FileName))
                    {
                        lock (unmodifiedProjects)
                        {
		...
		sealed class Wrapper : IBuildable2
		{
            ...
            public ProjectBuildOptions CreateProjectBuildOptions(
                BuildOptions options, bool isRootBuildable)
            {
            ...
            public ICollection<IBuildable> GetBuildDependencies(
                ProjectBuildOptions buildOptions)
            {
