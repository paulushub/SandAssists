...
namespace ICSharpCode.SharpDevelop
{
	public static class ParserService
	{
		...
		static Dictionary<IProject, IProjectContent> projectContents = 
            new Dictionary<IProject, IProjectContent>();
		static Dictionary<string, ParseInformation> parsings = 
            new Dictionary<string, ParseInformation>(StringComparer.OrdinalIgnoreCase);
		static ProjectContentRegistry defaultProjectContentRegistry = 
            new ProjectContentRegistry();
		...
		internal static void InitializeParserService()
		{
			if (parser == null) {
				...
				registries = AddInTree.BuildItems<ProjectContentRegistryDescriptor>(
                    "/Workspace/ProjectContentRegistry", null, false);
			...
			set {
				if (parser != null)
					throw new InvalidOperationException(
                        "Cannot set DomPersistencePath after ParserService was initialized");
		...
		static void InitAddedProject(object state)
		{
			...
			progressMonitor.BeginTask(
                "${res:ICSharpCode.SharpDevelop.Internal.ParserService.Parsing}...", 
                newContent.GetInitializationWorkAmount(), false);
			newContent.Initialize2(progressMonitor);
			progressMonitor.Done();
		}
		...
		{
			IList<string> defaultReferences = AddInTree.BuildItems<string>(
                "/SharpDevelop/Services/ParserService/SingleFileGacReferences", null, false);
		...
		public static string GetParseableFileContent(string fileName)
		{
			...
			if (file != null) {
				...
				if (p != null) {
					ITextDocument document = p.GetDocumentForFile(file);
					if (document != null) {
		...
		public static Encoding DefaultFileEncoding {
			get {
				return CodeEditorProperties.Instance.Encoding;
			}
		}
