...
namespace ICSharpCode.SharpDevelop
{
	public static class LanguageBindingService
	{
		...
		public static IProject LoadProject(IMSBuildEngineProvider provider, 
            string location, string title, string projectTypeGuid, 
            IProgressMonitor progressMonitor)
		{
			...
			if (!File.Exists(location)) {
				...
				if (binding != null) {
					...
					if (ext.Equals(".proj", StringComparison.OrdinalIgnoreCase)
					    || ext.Equals(".build", StringComparison.OrdinalIgnoreCase))
					{
