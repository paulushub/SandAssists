...
using ICSharpCode.TextEditor.Searching;
...
`				*/
		...
		static void AddDeclarationAsReference(List<Reference> list, string fileName, DomRegion region, string name)
		{
			if (fileName == null)
				return;
			SearchDocumentInfo documentInformation = FindReferencesAndRenameHelper.GetDocumentInformation(fileName);
		...
		public static SearchDocumentInfo GetDocumentInformation(string fileName)
		{
			...
			if (file != null) {
				...
				if (documentProvider != null) {
					ITextDocument document = documentProvider.GetDocumentForFile(file);
					if (document != null) {
						return new SearchDocumentInfo(document, fileName, 0);
					}
				}
			}
			ITextBufferStrategy strategy = StringTextBufferStrategy.CreateTextBufferFromFile(fileName);
			return new SearchDocumentInfo(strategy, fileName, 0);
		}
		...
		public static TextEditorControl JumpToDefinition(IMember member)
		{
			...
			ICSharpCode.SharpDevelop.TextEditor.Gui.ITextEditorControlProvider tecp = viewContent as ICSharpCode.SharpDevelop.TextEditor.Gui.ITextEditorControlProvider;
		...
		{
			public ICSharpCode.TextEditor.Document.ITextDocument Document;
			public int Offset;
			public int LengthDifference;
			
			public Modification(ICSharpCode.TextEditor.Document.ITextDocument document, int offset, int lengthDifference)
			{
		...
		public static void ModifyDocument(List<Modification> modifications, ICSharpCode.TextEditor.Document.ITextDocument doc, int offset, int length, string newName)
		{
		...
		public static void ShowAsSearchResults(string pattern, List<Reference> list)
		{
			...
			foreach (Reference r in list) {
				...
				res.SearchDocumentInfo = GetDocumentInformation(r.FileName);
				results.Add(res);
			}
		...
		{
			Dictionary<ICSharpCode.TextEditor.Document.ITextDocument, FileView> modifiedDocuments = new Dictionary<ICSharpCode.TextEditor.Document.ITextDocument, FileView>();
			List<Modification> modifications = new List<Modification>();
			foreach (Reference r in list) {
				ICSharpCode.TextEditor.Document.ITextDocument document = null;
				IViewContent viewContent = null;
			...
			foreach (KeyValuePair<ICSharpCode.TextEditor.Document.ITextDocument, FileView> entry in modifiedDocuments) {
		...
		{
			ICSharpCode.TextEditor.Document.ITextDocument doc = GetDocument(c);
		...
		public static ICSharpCode.TextEditor.Document.ITextDocument GetDocument(IClass c)
		{
