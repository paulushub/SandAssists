...
namespace ICSharpCode.SharpDevelop.Debugging
{
	public static class DebuggerService
	{
		...
		static DebuggerService()
		{
			...
			BookmarkExManager.Added   += BookmarkAdded;
			BookmarkExManager.Removed += BookmarkRemoved;
		}
		...
		public static IList<BreakpointBookmark> Breakpoints {
			get {
				...
				foreach (BookmarkEx bookmark in BookmarkExManager.Bookmarks) {
		...
		static void BookmarkAdded(object sender, BookmarkExEventArgs e)
		{
		...
		static void BookmarkRemoved(object sender, BookmarkExEventArgs e)
		{
			BreakpointBookmark bb = e.Bookmark as BreakpointBookmark;
			if (bb != null) {
				bb.RemoveMarker();
		...
		public static void ToggleBreakpointAt(ITextDocument document, string fileName, int lineNumber)
		{
            BookmarkManager marksManager = document.BookmarkManager;
            if (marksManager.DefaultMarker != BookmarkType.BreakMark)
            {
                // Avoid adding breakpoints to contents that do not support debugging...
                return;  
            }
			...
			for (int i = bookmarks.Count - 1; i >= 0; --i) {
				...
				if (breakpoint != null) {
                    ...
                    {
                        marksManager.RemoveMark(breakpoint);
                        return;
                    }
                }
            }
            ...
            foreach (char ch in document.GetText(document.GetLineSegment(lineNumber)))
            {
                ...
                {
                    marksManager.AddMark(new BreakpointBookmark(fileName, document, 
                        new TextLocation(column, lineNumber), BreakpointAction.Break, "", ""));
		...
		static string GetMemberText(IAmbience ambience, IEntity member, string expression, out bool debuggerCanShowValue)
		{
			...
			if (documentation != null && documentation.Length > 0) {
				text.Append('\n');
				text.Append(ICSharpCode.SharpDevelop.TextEditor.Gui.CodeCompletionData.ConvertDocumentation(documentation));
			}
