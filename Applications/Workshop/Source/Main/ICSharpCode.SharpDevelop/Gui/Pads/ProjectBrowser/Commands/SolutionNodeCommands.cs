...
namespace ICSharpCode.SharpDevelop.Project.Commands
{
	public class AddNewProjectToSolution : AbstractMenuCommand
	{
		...
		{
			BaseNode node = ProjectBrowserPad.Instance.ProjectBrowserControl.SelectedNode;
            ...
            {
				npdlg.InitialProjectLocationDirectory = 
                    GetInitialDirectorySuggestion(solutionFolderNode);
				
				// show the dialog to request project type and name
				if (npdlg.ShowDialog(WorkbenchSingleton.MainForm) == DialogResult.OK) 
                {
					if (npdlg.NewProjectLocation.Length == 0) 
                    {
						MessageService.ShowError(
                            "No project has been created, there is nothing to add.");
						return;
					}
					AddExitingProjectToSolution.AddProject(
                        solutionFolderNode, npdlg.NewProjectLocation);
	...
	public class AddExitingProjectToSolution : AbstractMenuCommand
	{
		public static void AddProject(ISolutionFolderNode solutionFolderNode, string fileName)
		{
			...
			AddProject(solutionFolderNode, LanguageBindingService.LoadProject(
                solutionFolderNode.Solution, fileName, 
