...
namespace ICSharpCode.SharpDevelop.Project
{
	...
	public class ProjectBrowserControl : UserControl, IHasPropertyContainer
	{
		...
		public bool ShowAll {
			...
			set {
				if (BaseNode.ShowAll != value) {
					treeView.BeginUpdate();
					BaseNode.ShowAll = value;
					foreach (BaseNode node in treeView.Nodes) {
		...
		/// <summary>
		/// The selected node or its most immediate parent that is a DirectoryNode.
		/// </summary>
		...
		public DirectoryNode SelectedDirectoryNode 
        {
			get 
            {
				TreeNode selectedNode =
					treeView.SelectedNode as BaseNode;
		...
		public BaseNode SelectedNode {
			get {
				return treeView.SelectedNode as BaseNode;
			}
		}
		...
		public BaseNode RootNode {
			get {
                if (treeView.Nodes.Count > 0)
                    return treeView.Nodes[0] as BaseNode;
		...
		{
			foreach (BaseNode treeNode in treeView.Nodes) {
		...
		public void RefreshView()
		{
			if (treeView.Nodes.Count > 0) {
				...
				ViewSolution(((BaseNode)treeView.Nodes[0]).Solution);
...
//
		...
		{
			BaseNode solutionNode = new SolutionNode(solution);
			treeView.Clear();
