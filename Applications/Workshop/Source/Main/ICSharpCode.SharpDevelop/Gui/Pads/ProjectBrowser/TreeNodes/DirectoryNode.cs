...
namespace ICSharpCode.SharpDevelop.Project
{
	...
	public class DirectoryNode : BaseNode, IOwnerState
	{
		...
		protected override void Initialize()
		{
			...
			Dictionary<FileNode, string> dependendFileDictionary = 
                new Dictionary<FileNode, string>();
			Dictionary<string, DirectoryNode> directoryNodeList = 
                new Dictionary<string, DirectoryNode>(StringComparer.OrdinalIgnoreCase);
			...
			foreach (KeyValuePair<FileNode, string> pair in dependendFileDictionary) {
				...
				BaseNode parentNode = fileNodeDictionary[fileName];
		...
		{
			if (relativeDirectoryPath.Length == 0
			    || String.Compare(virtualName, 0, relativeDirectoryPath, 0, 
                relativeDirectoryPath.Length, StringComparison.OrdinalIgnoreCase) == 0)
			{
