...
namespace ICSharpCode.SharpDevelop.Project.Commands
{
	public class AddReferenceToProject : AbstractMenuCommand
	{
		public override void Run()
		{
            ...
            if (command != null)
            {
                node = command.Caller as BaseNode;
            }     
	...
	public class RefreshWebReference : AbstractMenuCommand
	{
		public override void Run()
		{
            WebReferenceNode node = null;
            MenuCommand command = this.Owner as MenuCommand;
            if (command != null)
            {
                node = command.Caller as WebReferenceNode;
            }
	...
	public class AddWebReferenceToProject : AbstractMenuCommand
	{
		...
		void AddWebReferenceToProjectBrowser(BaseNode node, WebReference webReference)
		{
			TreeNode webReferencesNode = null;
			if (node is ProjectNode) {
				webReferencesNode = AddWebReferenceToProjectNode((ProjectNode)node, webReference);
			} else if (node is WebReferenceFolderNode) {
				webReferencesNode = node;
				WebReferenceNodeBuilder.AddWebReference((WebReferenceFolderNode)webReferencesNode, webReference);
			} else if (node is ReferenceFolderNode && node.Parent != null && node.Parent is ProjectNode) {
				...
				AddWebReferenceToProjectBrowser(node.Parent as BaseNode, webReference);
			}
		...
		TreeNode GetWebReferencesFolderNode(ProjectNode projectNode)
		{
			foreach (TreeNode node in projectNode.Nodes) {
				WebReferenceFolderNode webReferencesNode = node as WebReferenceFolderNode;
				if (webReferencesNode != null) {
	...
	public class RefreshReference : AbstractMenuCommand
	{
		public override void Run()
		{
            ...
            if (command != null)
            {
                node = command.Caller as ReferenceNode;
            }
