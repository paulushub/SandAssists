...
namespace ICSharpCode.SharpDevelop.Gui
{
	...
	public sealed class TextEditorSideBar : SharpDevelopSideBar
	{
		...
		{
            foreach (ClipTemplate template in ClipService.TextTemplates) 
            {
				SideTab tab = new SideTab(this, template.Name);
				tab.DisplayName = StringParser.Parse(tab.Name);
				tab.CanSaved  = false;
				foreach (ClipEntry entry in template.Entries)  
                {
					tab.Items.Add(SideTabItemFactory.CreateSideTabItem(
        ...
        private void LoadSideBarConfig(XmlElement el)
		{
        ...
        private XmlElement WriteConfig(XmlDocument doc)
		{
			if (doc == null) {
