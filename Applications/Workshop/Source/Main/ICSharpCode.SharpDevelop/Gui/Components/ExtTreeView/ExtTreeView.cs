...
namespace ICSharpCode.SharpDevelop.Gui
{
	...
	public class ExtTreeView : TreeView
	{
		...
		public ExtTreeView()
		{
            ...
            _mdHash = HashAlgorithm.Create("MD5");

            //// Tahoma font may not exist on the system...
            //try
            //{
            //    //Font ctrlFont = new Font("Segoe UI", 10);
            //    //Font ctrlFont = new Font("Trebuchet MS", 10);
            //    //Font ctrlFont = new Font("Verdana", 9);
            //    //Font ctrlFont = new Font("Arial Unicode MS", 10);
            ...
            this.Font = new Font(SystemFonts.MenuFont.Name, 10, FontStyle.Regular);
        }
		...
		{
			if (mouseClickNum == 2) {
				mouseClickNum = 0; // only intercept first occurrence, don't prevent collapsing by ActivateItem on double click
				e.Cancel = true;
				return;
			}
            ExtTreeNode node = e.Node as ExtTreeNode;  	
            if (node != null) {
                e.Cancel = !node.Collapsing();
			}
			...
			if (!inRefresh) {
				...
				if (node != null && !node.DrawDefault) {
					...
					if ((e.State & (TreeNodeStates.Selected | TreeNodeStates.Focused)) == TreeNodeStates.Selected) {
						// node is selected, but not focused:
						// HACK: work around TreeView bug in OwnerDrawText mode:
						// over-paint blue selection with the correct gray selection
						e.Graphics.FillRectangle(SystemBrushes.Control, e.Bounds);
						//e.Graphics.DrawString(e.Node.Text, this.Font, SystemBrushes.ControlText, e.Bounds.Location);
                        e.Graphics.DrawString(e.Node.Text, this.Font,
                            SystemBrushes.ControlText, e.Bounds, _textFormat);
						e.DrawDefault = false;
					} else {
						e.DrawDefault = true;
					}
		...
		public int GetImageIndexForFile(string imageFile, bool performCutBitmap)
		{
            Icon fileIcon = IconExtractor.Extract(imageFile, false);
            if (fileIcon == null)
            {
                // If the icon is not found, then we return to the default, missing
                // icon processing...
                return this.GetImageIndexForImage("Icons.16x16.MiscFiles", performCutBitmap);
            }

            // Compute a unique key, based on the file path to avoid conflict...
            Guid fileId = new Guid(_mdHash.ComputeHash(
                Encoding.UTF8.GetBytes(imageFile)));

            string image = fileId.ToString();
			string imageKey = performCutBitmap ? (image + "_ghost") : image;
			if (!imageIndexTable.ContainsKey(imageKey)) 
            {
                Bitmap fileImage = fileIcon.ToBitmap();

                fileIcon.Dispose();

                ImageList.Images.Add(performCutBitmap ? IconService.GetGhostBitmap(fileImage) : fileImage);
				imageIndexTable[imageKey] = ImageList.Images.Count - 1;
				return ImageList.Images.Count - 1;
			}
