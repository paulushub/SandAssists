...
namespace ICSharpCode.SharpDevelop.Gui
{
	public class ExtTreeNode : TreeNode, IDisposable, IClipboardHandler
	{
        ...
        protected StringFormat _textFormat;
		...
		public void SetIcon(string iconName)
		{
			...
			if (tree != null) 
            {
                ...
                { 
                    // It must be a missing file image, we attempt to provide a fix...
                    string fileExt = Path.GetExtension(iconName);
                    if (!String.IsNullOrEmpty(fileExt))
                    {
                        fileExt = fileExt.ToLower();
                    }

                    index = tree.GetImageIndexForFile(iconName, DoPerformCut);
                }
                else
                {
                    index = tree.GetImageIndexForImage(iconName, DoPerformCut);
                }
		...
		public virtual bool Collapsing()
		{
            return true;
		}
		...
		protected virtual void DrawForeground(DrawTreeNodeEventArgs e)
		{
            if (!drawDefault)
            {
                DrawText(e, this.Text, SystemBrushes.WindowText, TreeView.Font);
            }
		}
        ...
        protected void DrawText(DrawTreeNodeEventArgs e, string text, Brush brush, Font font, ref float x)
		{
            ...
            bounds.X = x + 3;
            g.DrawString(text, font, brush, bounds, _textFormat);
		...
		public static Font RegularBigFont {
			get {
				return regularBigFont ?? (regularBigFont = 
                    WinFormsResourceService.LoadFont(SystemFonts.MenuFont.Name, 10, FontStyle.Regular));
			}
		}
		...
		public static Font BoldBigFont {
			get {
				return boldBigFont
                    ?? (boldBigFont = WinFormsResourceService.LoadFont(SystemFonts.MenuFont.Name, 10, FontStyle.Bold));
			}
		}
		...
		public static Font ItalicBigFont {
			get {
				return italicBigFont
                    ?? (italicBigFont = WinFormsResourceService.LoadFont(SystemFonts.MenuFont.Name, 10, FontStyle.Italic));
			}
		}
		#endregion
