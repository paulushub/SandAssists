...
{
    public partial class ApplicationPanel : ProjectDialogPanel
    {
        ...
        public override void LoadPanelContents()
        {
            ...
            //    "ICSharpCode.SharpDevelop.Resources.ProjectOptions.ApplicationSettings.xfrm"));
            //SetupFromXmlResource("ICSharpCode.SharpDevelop.Resources.ProjectOptions.ApplicationSettings.xfrm");
            ...
            assemblyNameTextBox.TextChanged += new EventHandler(RefreshOutputNameTextBox);
            ...
            outputTypeComboBox.SelectedIndexChanged += RefreshOutputNameTextBox;
            outputTypeComboBox.SelectedIndexChanged += RefreshStartupObjectEnabled;
            ...
            applicationIconTextBox.TextChanged += new EventHandler(ApplicationIconTextBoxTextChanged);
            ...
            applicationManifestComboBox.Items.Add(StringParser.Parse(
                "${res:Dialog.ProjectOptions.ApplicationSettings.Manifest.EmbedDefault}"));
            applicationManifestComboBox.Items.Add(StringParser.Parse(
                "${res:Dialog.ProjectOptions.ApplicationSettings.Manifest.DoNotEmbedManifest}"));
            ...
            applicationManifestComboBox.Items.Add(StringParser.Parse(
                "<${res:Global.CreateButtonText}...>"));
            applicationManifestComboBox.Items.Add(StringParser.Parse(
                "<${res:Global.BrowseText}...>"));
            ...
            applicationManifestComboBox.TextChanged += delegate
            {
                helper.IsDirty = true;
            };
        ...
        void BrowseForManifest()
        {
            ...
            BrowseForFile(applicationManifestComboBox, 
                "${res:Dialog.ProjectOptions.ApplicationSettings.Manifest.ManifestFiles}|*.manifest|${res:SharpDevelop.FileFilter.AllFiles}|*.*", 
                TextBoxEditMode.EditEvaluatedProperty);
        }
        ...
        void CreateManifest()
        {
            ...
            if (!File.Exists(manifestFile))
            {
                ...
                using (Stream stream = typeof(ApplicationPanel).Assembly.GetManifestResourceStream(
                    "ICSharpCode.SharpDevelop.Properties.DefaultManifest.manifest"))
                {
                    if (stream == null)
                ...
                defaultManifest = defaultManifest.Replace("\t", CodeEditorProperties.Instance.IndentationString);
        ...
        private sealed class ManifestBinding : ConfigurationBinding
        {
        ...
        {                                                                    
            bool isLibrary = OutputType.Library == (OutputType)outputTypeComboBox.SelectedIndex;
            startupObjectComboBox.Enabled = !isLibrary;
        }

        void RefreshOutputNameTextBox(object sender, EventArgs e)
        {                                               
            outputNameTextBox.Text = assemblyNameTextBox.Text + 
                CompilableProject.GetExtension(
                (OutputType)outputTypeComboBox.SelectedIndex);
        }

        void ApplicationIconTextBoxTextChanged(object sender, EventArgs e)
        {                      
            if (FileUtility.IsValidPath(applicationIconTextBox.Text))
            {
                string applicationIcon = Path.Combine(baseDirectory, applicationIconTextBox.Text);
                if (File.Exists(applicationIcon))
                {
                    try
                    {                  
                        applicationIconPictureBox.Image = Image.FromFile(applicationIcon);
                    }
                    catch (OutOfMemoryException)
                    {
                        applicationIconPictureBox.Image = null;
                        MessageService.ShowErrorFormatted(
                            "${res:Dialog.ProjectOptions.ApplicationSettings.InvalidIconFile}", 
                            FileUtility.NormalizePath(applicationIcon));
                    }
                }
                else
                {
        ...
        protected void InitTargetFramework()
        {
            if (convertProjectToMSBuild35Button != null)
            {
                ...
                {
                    // VS05 project
                    this.targetFrameworkComboBox.Enabled = false;
                    convertProjectToMSBuild35Button.Click += OnConvertProjectToMSBuild35;
                    return;
                }
                else
                {
                    // VS08 project
                    this.targetFrameworkComboBox.Enabled = true;
                    convertProjectToMSBuild35Button.Visible = false;
                }
            }

            const string TargetFrameworkProperty = "TargetFrameworkVersion";
            ConfigurationBinding targetFrameworkBinding;

            targetFrameworkBinding = helper.BindStringEnum(
                this.targetFrameworkComboBox, TargetFrameworkProperty, "v2.0",
            ...
            targetFrameworkBinding.CreateLocationButton(this.targetFrameworkLabel);
        ...
        protected void OnConvertProjectToMSBuild35(object sender, EventArgs e)
        {
