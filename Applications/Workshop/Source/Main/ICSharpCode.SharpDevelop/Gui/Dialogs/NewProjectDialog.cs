	...
	public partial class NewProjectDialog : Form
	{
		...
		protected virtual void InitializeTemplates()
		{
			foreach (ProjectTemplate template in ProjectTemplate.ProjectTemplates) 
            {
                ...
                {
					Category cat = GetCategory(StringParser.Parse(
                        titem.Template.Category), 
                        StringParser.Parse(titem.Template.Subcategory));
			...
			if (templateListView.SelectedItems.Count == 1 && locationTextBox.Text.Length > 0 && solutionNameTextBox.Text.Length > 0) {
				...
				try {
					...
					MessageService.ShowError(
                        "${res:ICSharpCode.SharpDevelop.Gui.Dialogs.NewProjectDialog.CantCreateDirectoryError}");
					return;
				}
				...
				cinfo.ProjectBasePath = this.NewProjectDirectory;
				
				cinfo.SolutionName    = solution;
				cinfo.ProjectName     = name;
				
				this.NewSolutionLocation = item.Template.CreateProject(cinfo);
				if (String.IsNullOrEmpty(this.NewSolutionLocation)) 
                {
					return;
				}
				if (createNewSolution) {
					ProjectService.LoadSolution(this.NewSolutionLocation);
				}
				item.Template.RunOpenActions(cinfo);
				
				this.NewProjectLocation = cinfo.createdProjects.Count > 0 ? cinfo.createdProjects[0].FileName : "";
		...
		/// </summary>
		public sealed class TemplateItem : ListViewItem
		{
