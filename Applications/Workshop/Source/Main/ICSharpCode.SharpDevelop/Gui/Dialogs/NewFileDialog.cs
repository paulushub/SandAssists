...
namespace ICSharpCode.SharpDevelop.Gui
{
	...
	{
        /// <summary>
        /// We use an instance properties container to avoid "polluting" the 
        /// StringParser properties.
        /// </summary>
        private StringTagProvider _tagProvider;
        ...
        public NewFileDialog(string basePath)
        {
            ...
            _tagProvider = new StringTagProvider();
            _tagProvider["Number"] = "1";
            if (StringParser.Properties.ContainsKey("Number"))
            {
                _tagProvider["Number"] = StringParser.Properties["Number"];
            }
            ...
            foreach (Control panel in splitContainer.Controls)
            {
            ...
            foreach (Control ctl in bottomPanel.Controls)
            {
                ctl.Text = StringParser.Parse(ctl.Text);
            }
            ...
            StringParser.RegisterStringTagProvider(_tagProvider);
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            {
                StringParser.UnRegisterStringTagProvider(_tagProvider);
                _tagProvider = null;
            }
        ...
        private void InitializeView()
        {
            ...
            templateListView.HideSelection  = false;
        ...
        bool AllPropertiesHaveAValue
        {
            get
            {
                ...
                {
                    string val = _tagProvider["Properties." + property.Name];
        ...
        void ShowPropertyGrid()
        {
            ...
            if (propertySplitContainer.Panel2Collapsed)
            {
                //this.SuspendLayout();
                ...
                IList<TemplateProperty> properties = SelectedTemplate.Properties;
                int itemCount = properties.Count;
                for (int i = 0; i < itemCount; i++)
                {
                    TemplateProperty property = properties[i];
                    ...
                    if (property.Type.StartsWith("Types:"))
                    {
                        ...
                        _tagProvider["Properties." + localizedProperty.Name] = property.DefaultValue;
                        localizedProperty.DefaultValue = property.DefaultValue; // localizedProperty.TypeConverterObject.ConvertFrom();
                    }
                    else
                    {
                        localizedProperty = new LocalizedProperty(property.Name, property.Type, 
                        ...
                        if (property.Type == "System.Boolean")
                        {
                            ...
                            _tagProvider["Properties." + localizedProperty.Name] = defVal;
                            localizedProperty.DefaultValue = Boolean.Parse(defVal);
                        }
                        else if (property.Type == "System.Guid")
                        {
                            localizedProperty.TypeConverterObject = new GuidConverter();
                            string defVal = property.DefaultValue == null ? 
                                null : StringParser.Parse(property.DefaultValue.ToString());
                            if (defVal == null || defVal.Length == 0)
                            {
                                defVal = Guid.NewGuid().ToString();
                            }
                            _tagProvider["Properties." + localizedProperty.Name] = defVal;
                        ...
                        {
                            localizedProperty.TypeConverterObject = new DateTimeConverter();
                            string defVal = property.DefaultValue == null ? 
                                null : StringParser.Parse(property.DefaultValue.ToString());
                            if (defVal == null || defVal.Length == 0)
                            {
                                defVal = DateTime.Now.ToString();
                            }
                            _tagProvider["Properties." + localizedProperty.Name] = defVal;
                            localizedProperty.DefaultValue = defVal;
                        }
                        else if (property.Type == "System.Globalization.CultureInfo")
                        {
                            localizedProperty.TypeConverterObject = new CultureInfoConverter();
                            string defVal = property.DefaultValue == null ?
                                null : StringParser.Parse(property.DefaultValue.ToString());
                            if (defVal == null || defVal.Length == 0)
                            {
                                defVal = CultureInfo.InstalledUICulture.ToString();
                            }
                            _tagProvider["Properties." + localizedProperty.Name] = defVal;
                            localizedProperty.DefaultValue = defVal;
                        }
                        ...
                        {
                            string defVal = property.DefaultValue == null ? 
                                String.Empty : StringParser.Parse(property.DefaultValue.ToString());
                            _tagProvider["Properties." + localizedProperty.Name] = defVal;
                            localizedProperty.DefaultValue = defVal;
                        }
                    }

                    localizedProperty.SetReadOnly(property.IsReadOnly);

                    localizedProperty.LocalizedName = property.LocalizedName;
                    localizedProperty.TagProvider   = _tagProvider;
                ...
                propertySplitContainer.Panel2Collapsed = false;
            }
        }
        ...
        void HidePropertyGrid()
        {
            ...
            if (!propertySplitContainer.Panel2Collapsed)
            {
                propertySplitContainer.Panel2Collapsed = true;
                //this.SuspendLayout();
        ...
        string GenerateCurrentFileName()
        {
            if (SelectedTemplate.DefaultName.IndexOf("${Number}") >= 0)
            {
                try
                {
                    ...
                    while (true)
                    {
                        _tagProvider["Number"] = curNumber.ToString();
        ...
        void SelectedIndexChange(object sender, EventArgs e)
        {
            if (templateListView.SelectedItems.Count == 1)
            {
                descriptionLabel.Text = StringParser.Parse(SelectedTemplate.Description);
                openButton.Enabled = true;
                if (!this.allowUntitledFiles && !isNameModified)
                {
                    fileNameTextBox.Text = GenerateCurrentFileName();
        ...
        {
            templateListView.View = smallIconsRadioButton.Checked ? View.List : View.Tile;
        }
        ...
        public void SaveFile(FileDescriptionTemplate newfile, string content, byte[] binaryContent)
        {
            ...
            {
                if (CodeEditorProperties.Instance.IndentationString != "\t")
                {
                    parsedContent = parsedContent.Replace("\t", CodeEditorProperties.Instance.IndentationString);
                }
            }
        ...
        void OpenEvent(object sender, EventArgs e)
        {
            if (categoryTreeView.SelectedNode != null)
            {
                PropertyService.Set("Dialogs.NewProjectDialog.LargeImages", 
                    largeIconsRadioButton.Checked);
                PropertyService.Set("Dialogs.NewFileDialog.CategoryViewState", 
                    TreeViewHelper.GetViewStateString(categoryTreeView));
                PropertyService.Set("Dialogs.NewFileDialog.LastSelectedCategory", 
                    TreeViewHelper.GetPath(categoryTreeView.SelectedNode));
            }
            createdFiles.Clear();
            if (templateListView.SelectedItems.Count != 1)
            {
            ...
            if (!AllPropertiesHaveAValue)
            {
                MessageService.ShowMessage(
                    "${res:Dialog.NewFile.FillOutFirstMessage}", 
                    "${res:Dialog.NewFile.FillOutFirstCaption}");
                return;
            }
            ...
            _tagProvider["StandardNamespace"] = "DefaultNamespace";
            if (allowUntitledFiles)
            {
            ...
            else
            {
                ...
                if (project != null)
                {
                    _tagProvider["StandardNamespace"] = CustomToolsService.GetDefaultNamespace(project, fileName);
                }
            }
            _tagProvider["FullName"] = fileName;
            _tagProvider["FileName"] = Path.GetFileName(fileName);
            _tagProvider["FileNameWithoutExtension"] = Path.GetFileNameWithoutExtension(fileName);
            _tagProvider["Extension"] = Path.GetExtension(fileName);
            _tagProvider["Path"] = Path.GetDirectoryName(fileName);

            _tagProvider["ClassName"] = GenerateValidClassOrNamespaceName(Path.GetFileNameWithoutExtension(fileName), false);
            ...
            if (ProjectService.CurrentProject != null && !this.allowUntitledFiles)
            {
                ...
                foreach (ReferenceProjectItem reference in item.Template.RequiredAssemblyReferences)
                {
                    ...
                    if (!refs.Any(projItem => String.Equals(projItem.Include, 
                        reference.Include, StringComparison.OrdinalIgnoreCase)))
                    {
        ...
        /// </summary>
        public sealed class Category : TreeNode, ICategory
        {
            ArrayList categories = new ArrayList();
        ...
        /// </summary>
        private sealed class TemplateItem : ListViewItem
        {
        ...
        void InitializeComponents()
        {
            ...
            else
            {
                ...
                fileNameTextBox.TextChanged += new EventHandler(FileNameChanged);
            }
            ...
            largeIconsRadioButton.CheckedChanged += new EventHandler(CheckedChange);
            largeIconsRadioButton.FlatStyle = FlatStyle.Standard;
            ...
            smallIconsRadioButton.CheckedChanged += new EventHandler(CheckedChange);
            smallIconsRadioButton.FlatStyle = FlatStyle.Standard;
            ...
            tooltip.SetToolTip(largeIconsRadioButton, StringParser.Parse("${res:Global.SmallIconToolTip}"));
            tooltip.Active = true;
