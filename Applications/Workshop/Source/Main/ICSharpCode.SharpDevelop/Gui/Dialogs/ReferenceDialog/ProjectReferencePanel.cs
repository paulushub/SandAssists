...
namespace ICSharpCode.SharpDevelop.Gui
{
	public class ProjectReferencePanel : ListView, IReferencePanel
	{
		...
		void PopulateListView()
		{
            ...
            IEnumerable<IProject> projects = ProjectService.OpenSolution.Projects;
            foreach (IProject project in projects.OrderBy(
                p => p.Name, StringComparer.OrdinalIgnoreCase))
            {
                ListViewItem newItem = new ListViewItem(
                    new string[] { project.Name, project.Directory });
				newItem.Tag = project;
				Items.Add(newItem);
			}
		}
