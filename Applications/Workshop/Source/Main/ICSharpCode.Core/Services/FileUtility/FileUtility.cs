...
namespace ICSharpCode.Core
{
	...
	public static partial class FileUtility
	{
        ...
        /// <summary>
        /// This removes the end back slash, if any from a directory.
        /// <para>
        /// eg: "c:\tmp\" to "c:\tmp"
        /// </para>
        /// </summary>
...
//				erg.Append(Path.DirectorySeparatorChar);
		...
		/// </summary>
		static void SearchDirectory(string directory, string filemask, 
            List<string> collection, bool searchSubdirectories, bool ignoreHidden)
		{
            ...
            try
            {
                ...
                {
                    if (ignoreHidden && (File.GetAttributes(f) & FileAttributes.Hidden) ==
		...
		{
			// Fix-me: 260 is the hard-coded maximal length for a path on my Windows XP system
		...
		public static bool TestFileExists(string filename)
		{
			if (!File.Exists(filename)) {
				MessageService.ShowWarning(StringParser.Parse(
                    "${res:Fileutility.CantFindFileError}", 
		...
		public static FileOperationResult ObservedSave(FileOperationDelegate saveFile, 
            string fileName, string message, FileErrorPolicy policy)
		{
			...
			try {
				...
				switch (policy) {
					...
					case FileErrorPolicy.ProvideAlternative:
						ChooseSaveErrorResult r = 
                            ServiceManager.MessageService.ChooseSaveError(fileName, 
                            message, "${res:FileUtilityService.ErrorWhileSaving}", e, false);
						if (r.IsRetry) {
		...
		public static FileOperationResult ObservedSave(FileOperationDelegate saveFile, 
            string fileName, FileErrorPolicy policy)
		{
			return ObservedSave(saveFile, fileName, ResourceService.GetString(
                "ICSharpCode.Services.FileUtilityService.CantSaveFileStandardText"),
			                    policy);
		}
		...
		public static FileOperationResult ObservedSave(
            NamedFileOperationDelegate saveFileAs, string fileName, string message, 
            FileErrorPolicy policy)
		{
			...
			try {
				...
				switch (policy) {
					case FileErrorPolicy.Inform:
						ServiceManager.MessageService.InformSaveError(fileName, message, 
                            "${res:FileUtilityService.ErrorWhileSaving}", e);
						break;
					case FileErrorPolicy.ProvideAlternative:
						ChooseSaveErrorResult r = 
                            ServiceManager.MessageService.ChooseSaveError(fileName, 
                            message, "${res:FileUtilityService.ErrorWhileSaving}", 
                            e, true);
						if (r.IsRetry) {
							...
							return ObservedSave(saveFileAs, r.AlternativeFileName, 
                                message, policy);
						}
						break;
				}
		...
		public static FileOperationResult ObservedSave(
            NamedFileOperationDelegate saveFileAs, string fileName, 
            FileErrorPolicy policy)
		{
			return ObservedSave(saveFileAs, fileName, ResourceService.GetString(
                "ICSharpCode.Services.FileUtilityService.CantSaveFileStandardText"),
			                    policy);
		}
		...
		public static FileOperationResult ObservedLoad(FileOperationDelegate loadFile, 
            string fileName, string message, FileErrorPolicy policy)
		{
			try {
				loadFile();
                RaiseFileLoaded(new FileNameEventArgs(fileName));
				return FileOperationResult.OK;
			} catch (Exception e) {
				switch (policy) {
					case FileErrorPolicy.Inform:
						ServiceManager.MessageService.InformSaveError(fileName, message, 
                            "${res:FileUtilityService.ErrorWhileLoading}", e);
						break;
					case FileErrorPolicy.ProvideAlternative:
						ChooseSaveErrorResult r = 
                            ServiceManager.MessageService.ChooseSaveError(fileName, 
                            message, "${res:FileUtilityService.ErrorWhileLoading}", 
                            e, false);
						if (r.IsRetry)
		...
		public static FileOperationResult ObservedLoad(FileOperationDelegate loadFile, 
            string fileName, FileErrorPolicy policy)
		{
			return ObservedLoad(loadFile, fileName, ResourceService.GetString(
                "ICSharpCode.Services.FileUtilityService.CantLoadFileStandardText"),
			                    policy);
		}
		...
		public static FileOperationResult ObservedLoad(
            ...
            FileErrorPolicy policy)
		{
			return ObservedLoad(new FileOperationDelegate(delegate { saveFileAs(fileName); }), 
		...
		public static FileOperationResult ObservedLoad(
            ...
            FileErrorPolicy policy)
		{
			return ObservedLoad(saveFileAs, fileName, ResourceService.GetString(
                "ICSharpCode.Services.FileUtilityService.CantLoadFileStandardText"),
			                    policy);
		}
        ...
        static void RaiseFileLoaded(FileNameEventArgs e)
		{
			if (FileLoaded != null) {
				FileLoaded(null, e);
			}
