	...
	public static class AddInManager
	{
		...
		public static void LoadAddInConfiguration(List<string> addInFiles, List<string> disabledAddIns)
		{
			...
			using (XmlReader reader = XmlReader.Create(configurationFileName, settings)) 
            {
				while (reader.Read()) 
                {
                    ...
                    {
						if (String.Equals(reader.Name, "AddIn", 
                        ...
                        else if (String.Equals(reader.Name, "Disable", 
                            StringComparison.OrdinalIgnoreCase)) 
                        {
		...
		public static void SaveAddInConfiguration(List<string> addInFiles, List<string> disabledAddIns)
		{
			...
			using (XmlWriter writer = XmlWriter.Create(configurationFileName, settings)) 
            {
                ...
                int itemCount = addInFiles.Count;
                for (int i = 0; i < itemCount; i++)
                {
                    writer.WriteStartElement("AddIn");
                    writer.WriteAttributeString("file", addInFiles[i]);
                    writer.WriteEndElement();
                }

                itemCount = disabledAddIns.Count;
                for (int i = 0; i < itemCount; i++)
                {
                    writer.WriteStartElement("Disable");
                    writer.WriteAttributeString("addin", disabledAddIns[i]);
                    writer.WriteEndElement();
                }
