...
namespace ICSharpCode.SharpDevelop.Dom.VBNet
{
	public class VBExpressionFinder : IExpressionFinder
	{
		ExpressionResult CreateResult(string expression)
		{
			...
			if (expression.Length > 8 && expression.Substring(0, 8).Equals("Imports ", StringComparison.OrdinalIgnoreCase))
				return new ExpressionResult(expression.Substring(8).TrimStart(), ExpressionContext.Type);
			if (expression.Length > 4 && expression.Substring(0, 4).Equals("New ", StringComparison.OrdinalIgnoreCase))
				return new ExpressionResult(expression.Substring(4).TrimStart(), ExpressionContext.ObjectCreation);
			if (curTokenType == Ident && lastIdentifier.Equals("as", StringComparison.OrdinalIgnoreCase))
			...
			switch (ch)
			{
				...
				default:
					if (IsIdentifierPart(ch))
					{
						...
						if (ident != null) {
							if (string.Equals(ident, "new", StringComparison.OrdinalIgnoreCase)) {
								curTokenType = New;
							} else if (string.Equals(ident, "imports", StringComparison.OrdinalIgnoreCase)) {
								curTokenType = Using;
							} else {
