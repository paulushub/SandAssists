...
namespace ICSharpCode.SharpDevelop.Widgets.TabControls
{
	...
	/// </summary>
	[Designer( typeof( ICSharpCode.SharpDevelop.Widgets.TabControls.Design.YaTabControlDesigner ) )]
	public class YaTabControl : Control
	{
		...
		/// </summary>
		private List<float> yaTabLengths;

        /// <summary>
        /// The array of tab pages added to this control.
        /// </summary>
        private Collection<YaTabPage> yaTabPages;
        ...
        public YaTabControl()
        {
            yaTabPages = new YaControlCollection(this);

            SetStyle(ControlStyles.UserPaint | ControlStyles.AllPaintingInWmPaint | ControlStyles.DoubleBuffer, true);
            defaultImageIndex = -1;
            yaTabLengths = new List<float>(5);
            ...
            yaActiveBrush = (Brush)SystemBrushes.ControlLight.Clone();
            yaActiveColor = SystemColors.ControlLight;
            yaInactiveBrush = (Brush)SystemBrushes.Window.Clone();
            yaInactiveColor = SystemColors.Window;
            if (Environment.OSVersion.Version.Major >= 6)
            {
                yaBorderPen = new Pen(SystemColors.Highlight, 1f);
            }
            else
            {   
        ...
        public virtual int SelectedIndex
        {
            ...
            set
			{
                ...
                if (yaTabPages.Count > 0)
				{
                    if (yaSelectedTab != null)
                    {
                        yaSelectedTab.Visible = false;
                    }
                    yaSelectedTab = yaTabPages[value];
		...
		public virtual YaTabPage SelectedTab
		{
			...
			set
			{
				...
				{
					int newIndex;
                    for (newIndex = 0; newIndex < yaTabPages.Count; newIndex++)
					{
                        if (value == yaTabPages[newIndex])
						{
							break;
						}
					}
        ...
        /// <summary>
        /// Gets the list of tab pages on this control.
        /// </summary>
        /// <value>
        /// A <see cref="IList{T}"/> of <see cref="YaTabPage"/> specifyig all the
        /// available tab pages.
        /// </value>
        public IList<YaTabPage> TabPages
        {
            get
            {
                return yaTabPages;
		...
		{
            if (index < 0 || index >= yaTabPages.Count)
			{
			...
			{
				l = Height - yaTabLengths[ 0 ] + yaTabLeftDif;
		...
		public virtual void ScrollTabs( int amount )
		{
			...
			if( yaTabLeftDif <= 0 || yaTabLeftDif >= yaTotalTabSpan - yaTabLengths[ yaTabLengths.Count - 1 ])
			{
				lock( this )
				{
			...
			if( yaTabLeftDif >= yaTotalTabSpan - yaTabLengths[ yaTabLengths.Count - 1 ])
			{
				canScrollLeft = false;
			}
			if( yaTabLeftDif <= 0 )
		...
		protected override void OnPaint( PaintEventArgs pea )
		{
            ...
            if (yaTabPages.Count > 0)
			{
				...
				switch( yaTabDock )
				{
					case DockStyle.Bottom:
						invert = true;
						graphics.TranslateTransform( Convert.ToSingle( calcWidth ), Convert.ToSingle( calcHeight ) );
						graphics.RotateTransform( 180.0f );
						break;
					case DockStyle.Left:
						graphics.TranslateTransform( 0, Convert.ToSingle( Height ) );
						graphics.RotateTransform( -90.0f );
						break;
					case DockStyle.Right:
						graphics.TranslateTransform( Convert.ToSingle( Width ), 0 );
						graphics.RotateTransform( 90.0f );
						break;
				}

				// Paint the areas.
				graphics.FillRectangle( yaInactiveBrush, yaTabsRectangle );
				graphics.FillRectangle( yaActiveBrush, yaClientRectangle );

				// Draws the highlight/shadow line, if applicable.
				Pen p = yaBorderPen;
				if( yaTabDrawer != null && yaTabDrawer.UsesHighlights )
				{
				...
				graphics.DrawLine( p, 0, yaClientRectangle.Y, calcWidth, yaClientRectangle.Y );
				...
				if( yaTabDrawer != null )
				{
					if( !invert )
					{
						graphics.TranslateTransform( 2.0f * Convert.ToSingle( yaMargin ) - yaTabLeftDif, yaMargin + 1.0f );
					}
					else
					{
						graphics.TranslateTransform( Convert.ToSingle( Width ) - 2.0f * yaMargin - yaTabLengths[ 0 ] + yaTabLeftDif, yaMargin + 1.0f );
					}
					...
					for ( int i = 0; i <= yaLastVisibleTabIndex; i++ )
					{
						s.Width = yaTabLengths[ i ];
						if( i != yaSelectedIndex )
						{
							yaTabDrawer.DrawTab( yaActiveColor, yaInactiveColor, 
                                yaHighlightPen.Color, yaShadowPen.Color, 
                                yaBorderPen.Color, false, yaTabDock, graphics, s );
						}
						else
						{
							selTransform = graphics.Transform;
						}
						if( invert )
						{
							if( i + 1 < yaTabLengths.Count )
							{
								graphics.TranslateTransform( -yaTabLengths[ i + 1 ], 0.0f );
							}
						}
						else
						{
							graphics.TranslateTransform( s.Width, 0.0f );
						}
					}
					...
					{
						graphics.Transform = selTransform;
						s.Width = yaTabLengths[ yaSelectedIndex ];
						yaTabDrawer.DrawTab( yaActiveColor, yaInactiveColor, 
                            yaHighlightPen.Color, yaShadowPen.Color, 
                            yaBorderPen.Color, true, yaTabDock, graphics, s );
					}
				}
				...
				if( !invert )
				{
					...
					graphics.TranslateTransform( 2.0f * yaMargin - yaTabLeftDif, yaMargin );
				}
				else
				{
					graphics.ResetTransform();
					graphics.TranslateTransform( 2.0f * yaMargin - yaTabLeftDif, yaClientRectangle.Height );
				}
				YaTabPage ytp = null;
				for( int i = 0; i <= yaLastVisibleTabIndex; i++ )
				{
					...
					{
						graphics.DrawImage( images.Images[ ytp.ImageIndex ], 1.5f * yaMargin, yaMargin );
						if( i != yaSelectedIndex )
						{
                            graphics.DrawString(yaTabPages[i].Text, yaTabFont, yaForeBrush, 2.5f * yaMargin + images.Images[ytp.ImageIndex].Width, 1.5f * yaMargin);
						}
						else
						{
                            graphics.DrawString(yaTabPages[i].Text, yaBoldTabFont, yaForeBrush, 2.5f * yaMargin + images.Images[ytp.ImageIndex].Width, 1.5f * yaMargin);
						}
					}
					...
					{
						graphics.DrawImage( images.Images[ defaultImageIndex ], 1.5f * yaMargin, yaMargin );
						if( i != yaSelectedIndex )
						{
                            graphics.DrawString(yaTabPages[i].Text, yaTabFont, yaForeBrush, 2.5f * yaMargin + images.Images[defaultImageIndex].Width, 1.5f * yaMargin);
						}
						else
						{
                            graphics.DrawString(yaTabPages[i].Text, yaBoldTabFont, yaForeBrush, 2.5f * yaMargin + images.Images[defaultImageIndex].Width, 1.5f * yaMargin);
						}
					}
					else
					{
						if( i != yaSelectedIndex )
						{
                            graphics.DrawString(yaTabPages[i].Text, yaTabFont, yaForeBrush, 1.5f * yaMargin, 1.5f * yaMargin);
						}
						else
						{
                            graphics.DrawString(yaTabPages[i].Text, yaBoldTabFont, yaForeBrush, 1.5f * yaMargin, 1.5f * yaMargin);
						}
					}
					graphics.TranslateTransform( s.Width, 0 );
				}
				...
				if( yaShowScrollButtons == YaScrollButtonStyle.Always || ( yaShowScrollButtons == YaScrollButtonStyle.Auto && yaTotalTabSpan > calcWidth ) )
				{
					...
					{
						graphics.ResetTransform();
						graphics.TranslateTransform( 0, yaClientRectangle.Height );
					}
					else
					{
						graphics.Transform = m;
					}
					graphics.FillRectangle( yaInactiveBrush, calcWidth - 2 * yaTabsRectangle.Height, 0, 2 * yaTabsRectangle.Height, yaTabsRectangle.Height );
					graphics.DrawRectangle( yaBorderPen, calcWidth - 2 * yaTabsRectangle.Height, 0, 2 * yaTabsRectangle.Height, yaTabsRectangle.Height );
					if( ( ( yaShowScrollButtons == YaScrollButtonStyle.Always && 
                        yaTotalTabSpan > calcWidth - 2 * Convert.ToInt32( yaTabsRectangle.Height ) ) || 
                        ( yaShowScrollButtons == YaScrollButtonStyle.Auto && 
                        yaTotalTabSpan > calcWidth ) ) && 
                        yaTabLeftDif < yaTotalTabSpan - yaTabLengths[yaTabLengths.Count - 1 ])
					{
						canScrollLeft = true;
						graphics.FillPolygon( yaBorderPen.Brush, leftArrow );
					}
					if( yaTabLeftDif > 0 )
					{
						canScrollRight = true;
						graphics.FillPolygon( yaBorderPen.Brush, rightArrow );
					}
					graphics.DrawPolygon( yaBorderPen, leftArrow );
					graphics.DrawPolygon( yaBorderPen, rightArrow );
				}
			}

			// Reset the transform and draw the border.
			graphics.ResetTransform();
			graphics.DrawRectangle( yaBorderPen, 0, 0, ClientRectangle.Width - 1, ClientRectangle.Height - 1 );
		}
		...
		protected override void OnSizeChanged( EventArgs e )
		{
			...
			CalculateRectangles();
			CalculateLastVisibleTabIndex();
			if( yaTabLengths.Count > 0 && yaTabLeftDif >= yaTotalTabSpan - yaTabLengths[ yaTabLengths.Count - 1 ])
			{
				yaTabLeftDif = 0;
				ScrollTabs( -Convert.ToInt32( yaTotalTabSpan - yaTabLengths[ yaTabLengths.Count - 1 ]));
			}
			PerformLayout();
			InU();
		}
        ...
        {
            return new YaDefaultControlCollection(this);
        }
		...
		private void CalculateTabLengths()
		{
			...
			float f = 0.0f;
			YaTabPage ytp;
            for (int i = 0; i < yaTabPages.Count; i++)
			{
                f = g.MeasureString(yaTabPages[i].Text, yaBoldTabFont).Width + 4.0f * Convert.ToSingle(yaMargin);
                ytp = yaTabPages[i];
				if( ytp != null && images != null && ytp.ImageIndex > -1 && ytp.ImageIndex < images.Images.Count && images.Images[ ytp.ImageIndex ] != null )
				{
        ...
        private class YaControlCollection : Collection<YaTabPage>
        {
            /// <summary>
            ...
            public YaControlCollection(YaTabControl owner)
            {
                this.owner = owner;
                monitor = new EventHandler(this.owner.ChildTabTextChanged);
            }

            protected override void InsertItem(int index, YaTabPage item)
            {
                item.SendToBack();
                //base.Add( p );
                item.TextChanged += monitor;
            ...
            protected override void SetItem(int index, YaTabPage item)
            {
                item.SendToBack();
                //base.Add( p );
                item.TextChanged += monitor;
		...
		/// </summary>
		private class YaDefaultControlCollection : Control.ControlCollection
		{
			/// <summary>
			/// Creates a new instance of the
			...
			/// </exception>
            public YaDefaultControlCollection(Control owner)
                : base(owner)
			{
			}
