...
{			
	public class SideBarControl : UserControl
    {
        ...
        private SideTab activeTab;
		private SideTabCollection sideTabs;

        private SideTabRenderer _renderer;
        private SideTabRenderer _contentRenderer;
		
		private SideTabContent sideTabContent;
		
		private SideTab renameTab;
		private SideTabItem renameTabItem;
		private TextBox renameTextBox;
		
		private ScrollBar scrollBar;
		
		private Point mousePosition;
		private bool  doAddTab;

        private ISideTabFactory sideTabFactory;
        private ISideTabItemFactory sideTabItemFactory;

        private MouseWheelHandler mouseWheelHandler;

        private Point mouseDownPosition;
        private SideTab mouseDownTab;

        private Rectangle activeTabMemberArea;
		...
		{    
            ...
            sideTabContent     = new SideTabContent();

            _renderer          = new SideTabRenderer();
            _contentRenderer   = new SideTabRenderer();
            sideTabFactory     = new DefaultSideTabFactory();
            sideTabItemFactory = new DefaultSideTabItemFactory();
            ...
            // Tahoma font may not exist on the system...
            try
            {
                Font ctrlFont = new Font("Tahoma", 10);
        ...
        public SideTabRenderer TabRenderer
        {
            get
            {
        ...
        protected SideTabContent TabContent
        {
            get
            {
                return sideTabContent;
            }
        }
			...
			if (e.Data.GetDataPresent(typeof(SideTabItem))) {
				...
				if (tab != null) {
					if (tab == Tabs.DragOverTab && tab.CanDragDrop) {
						...
						draggedItem.Status = SideTabItemStatus.Normal;
						switch (e.Effect) {
		...
		protected override void OnPaint(PaintEventArgs e)
		{
            ...
            _renderer.Initialize(g, this.Font);
			...
			for (; i < sideTabs.Count; ++i) {
				...
				int yPos = i * (Font.Height + 4 + 1);
                _renderer.DrawHeader(tab, new Point(0, yPos), Width);
				lastUpperY = yPos + Font.Height + 4;
			...
			if (activeTab != null) 
            {
				bool b = scrollBar.Maximum > (bottom - lastUpperY) / 20 || 
                    scrollBar.Value != 0;
				scrollBar.Visible  = b;
				activeTabMemberArea = new Rectangle(0, lastUpperY, this.Width - 
                    (scrollBar.Visible ? (SystemInformation.VerticalScrollBarWidth) : 0)  - 4, bottom - lastUpperY);
        ...
        private void ClearDraggings(SideTab tab)
        {
            foreach (SideTabItem item in tab.Items)
            {
                if (item.Status == SideTabItemStatus.Drag)
                {
                    item.Status = SideTabItemStatus.Normal;
                }
            }
        }

        private void ItemContextMenuPopup(object sender, EventArgs e)
        {
        ...
        protected sealed class SideTabContent : UserControl
		{
			...
			public SideBarControl SideBar {
				...
				set {
                    ...
                    if (value != null)
                    {
                        _contentRenderer = value.TabRenderer;
                    }
			...
			protected override void OnPaint(PaintEventArgs e)
			{
                ...
                {
                    _contentRenderer.Initialize(g, this.Font);

                    sideBar.activeTab.DrawTabContent(_contentRenderer, 
                        new Rectangle(0, 0, Width, Height));

                    _contentRenderer.Uninitialize();
				}
			}
			...
			protected override void OnDragDrop(DragEventArgs e)
			{
				...
				if (e.Data.GetDataPresent(typeof(SideTabItem))) {
					...
					switch (e.Effect) {
						...
						case DragDropEffects.Copy:
							SideTabItem newItem = draggedItem.Clone();
							newItem.Status = SideTabItemStatus.Normal;
				...
				if (e.Data.GetDataPresent(typeof(SideTabItem))) {
					...
					if (sideBar.activeTab.CanDragDrop) {
						...
						if (item == null) {
							...
							if (item.Status != SideTabItemStatus.Drag) {
								sideBar.ClearDraggings(sideBar.activeTab);
								item.Status = SideTabItemStatus.Drag;
