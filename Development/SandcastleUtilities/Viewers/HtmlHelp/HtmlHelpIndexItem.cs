...
namespace Sandcastle.Viewers.HtmlHelp
{
	...
	/// </summary>
	public sealed class HtmlHelpIndexItem : IComparable
	{
		/// <summary>
		...
		/// </summary>
        private List<HtmlHelpIndexTopic> _Topics = null;
		/// <summary>
		...
		internal HtmlHelpIndexItem(CHMFile chmFile, string keyWord, bool isSeeAlso, 
            int indent, int charIndex, int entryIndex, string[] seeAlsoValues, 
            int[] topicOffsets)
		{
			_chmFile = chmFile;
		...
		internal void Dump(ref BinaryWriter writer, bool writeFileName)
		{
			...
			{
				HtmlHelpIndexTopic topic = ((HtmlHelpIndexTopic)(Topics[i]));
		...
		internal bool ReadDump(ref BinaryReader reader, IList<CHMFile> filesList)
		{
			int i=0;
			...
			for(i=0; i<nCnt; i++)
			{
				HtmlHelpIndexTopic topic = new HtmlHelpIndexTopic("","","","");
        ...
        public IList<HtmlHelpIndexTopic> Topics
		{
			get 
			{
				if( _Topics == null )
				{
					if(IsSeeAlso)
					{
                        _Topics = new List<HtmlHelpIndexTopic>();
					} 
					else 
					{
						if( (_chmFile != null) && (_chmFile.TopicsFile != null) )
						{
							...
							{
								HtmlHelpIndexTopic newTopic = HtmlHelpIndexTopic.FromTopicEntry((TopicEntry)_chmFile.TopicsFile.TopicTable[ _nTopics[i] ]);
