...
namespace Sandcastle.Viewers.HtmlHelp.Decoding
{
	...
	internal sealed class FullTextEngine : IDisposable
	{
		...
		/// </summary>
        private List<HitHelper> _hitsHelper = new List<HitHelper>();
		/// <summary>
		...
		public bool Search(string search, int MaxHits, bool partialMatches, bool titleOnly)
		{
			if(CanSearch)
			{
				...
				_hitsHelper = null;
                _hitsHelper = new List<HitHelper>();
				...
				if(bRet)
				{
					...
					{
						HitHelper curHlp = _hitsHelper[i-1];
		...
		private void FinalizeQuoted(string search)
		{
			...
			while( quoteRE.IsMatch(search, nIndex) )
			{
				...
				{
					if( ! CheckHit(_hitsHelper[i], wordsInPhrase) )
