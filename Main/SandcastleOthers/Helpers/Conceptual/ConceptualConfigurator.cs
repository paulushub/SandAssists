...
namespace Sandcastle.Conceptual
{
    ...
    public class ConceptualConfigurator : AssemblerConfigurator
    {
        ...
        private ConceptualEngine    _engine;
        ...
        /// </value>
        protected override BuildEngine Engine
        {
            get
            {
                return _engine;
        ...
        public void Initialize(ConceptualEngine engine)
        {
            ...
            IList<BuildFormat> listFormats = _settings.OutputFormats;
        ...
        public override void Initialize(BuildEngine engine)
        {
            this.Initialize(engine as ConceptualEngine);
        }
        ...
        protected void OnClonedInclude(ConfigurationItemEventArgs args)
        {
            ...
            {
                _singleFormat.WriteAssembler(_engine, _group, xmlWriter);
            }
            else
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    {   
                        ...
                        format.WriteAssembler(_engine, _group, xmlWriter);
        ...
        protected void OnMetadataAttributesItem(object sender, ConfigurationItemEventArgs args)
        {
            ...
            // For now, lets simply write the default...
            xmlWriter.WriteStartElement("data");
            xmlWriter.WriteAttributeString("files", @".\ExtractedFiles\ContentMetadata.xml");
            xmlWriter.WriteEndElement();
        ...
        protected void OnMetadataSettingsItem(object sender, ConfigurationItemEventArgs args)
        {
            ...
            // For now, lets simply write the default...
            xmlWriter.WriteStartElement("data");
            xmlWriter.WriteAttributeString("files", @".\ExtractedFiles\Projectsettings.xml");
            xmlWriter.WriteEndElement();
        ...
        protected void OnTransformsItem(object sender, ConfigurationItemEventArgs args)
        {
            ...
            {
                throw new InvalidOperationException("A document transformer is required.");
            }
        ...
        protected void OnSharedContentsItem(object sender, ConfigurationItemEventArgs args)
        {
            ...
            sharedContents = _settings.GetSharedContents(BuildEngineType.Conceptual);

            if (sharedContents != null && sharedContents.Count != 0)
            {
                SharedContentConfigurator configurator = new SharedContentConfigurator();
                configurator.Initialize(_engine);
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    if (String.IsNullOrEmpty(sharedContent) == false)
                    {
                        string fileName = "TopicsSharedContent.xml";
                        if (itemCount > 1)  // not yet the case...
                        {
                            fileName = String.Format("TopicsSharedContent{0}.xml", i);
                        }
                        string finalSharedContent = Path.Combine(workingDir, fileName);

                        configurator.Configure(sharedContent, finalSharedContent);
        ...
        #region OnItem Method

        private void OnTempItem(object sender, ConfigurationItemEventArgs args)
        {
            XPathNavigator navigator = args.Navigator;
