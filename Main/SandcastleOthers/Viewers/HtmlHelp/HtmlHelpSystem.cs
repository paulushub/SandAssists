...
namespace Sandcastle.Viewers.HtmlHelp
{
	...
	public sealed class HtmlHelpSystem
	{
		...
		/// </summary>
        private List<CHMFile> _chmFiles = new List<CHMFile>();
		/// <summary>
		/// Internal member storing a merged table of contents
		/// </summary>
		private HtmlHelpToc _toc = new HtmlHelpToc();
		/// <summary>
		/// Internal member storing a merged index
		/// </summary>
		private HtmlHelpIndex _index = new HtmlHelpIndex();
		/// <summary>
		...
		/// </summary>
		/// <param name="tocs">HtmlHelpTocItem list</param>
		/// <param name="merged">Arraylist which holds the merged-items</param>
        internal void MarkChildrenAdded(IList<HtmlHelpTocItem> tocs, IList<HtmlHelpTocItem> merged)
		{
			foreach(HtmlHelpTocItem curItem in tocs)
			{
		...
		internal void RemoveMergeLinks()
		{
			foreach(CHMFile curFile in _chmFiles)
			{
				if( curFile.MergLinks.Count > 0)
				{
					...
					{
						HtmlHelpTocItem curItem = curFile.MergLinks[0] as HtmlHelpTocItem;
		...
		public bool HasInformationTypes
		{
			get 
            { 
                return (_informationTypes.Count > 0); 
            }
		}
		...
		/// </summary>
		public IList<InformationType> InformationTypes
		{
		...
		/// </summary>
		public HtmlHelpToc HtmlHelpToc
		{
			get
			{
				if( _chmFiles.Count > 0 )
