...
namespace Sandcastle.Viewers.HtmlHelp.Decoding
{
	...
	internal sealed class CHMSystem : IDisposable
	{
		...
		private string GetMasterHHC(IList<string> hhcTopics)
		{
			string sRet = "";
						...
						if( (_associatedFile != null) && (_associatedFile.TopicsFile != null ) )
						{
							...
							if( te == null)
							{
								...
								if( te == null)
								{
									...
									if( te == null)
									{
                                        IList<TopicEntry> arrExt = _associatedFile.TopicsFile.GetByExtension("hhc");

										if( arrExt == null )
										{
											IList<string> listExts = _associatedFile.EnumFilesByExtension("hhc");
											...
											else 
											{
                                                if (listExts.Count > 1)
												{
                                                    sCheck = GetMasterHHC(listExts);
													_contentsFile = sCheck;
												} 
												else 
												{
													_contentsFile = listExts[0];
										...
										else 
										{
											if(arrExt.Count > 1)
											{
												sCheck = GetMasterHHC(arrExt);
												_contentsFile = sCheck;
											} 
											else 
											{
												_contentsFile = arrExt[0].Locale;
												sCheck = _contentsFile;
											}
										}
									} 
					...
					if(_indexFile.Length <= 0)
					{
						...
						if( (_associatedFile != null) && (_associatedFile.TopicsFile != null ) )
						{
							...
							if( te == null)
							{
								...
								if( te == null)
								{
                                    List<TopicEntry> arrExt = _associatedFile.TopicsFile.GetByExtension("hhk");
