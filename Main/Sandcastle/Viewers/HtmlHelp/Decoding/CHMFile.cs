...
using Sandcastle.Viewers.HtmlHelp.Storage;
...
namespace Sandcastle.Viewers.HtmlHelp.Decoding
{
	...
	public sealed class CHMFile : IDisposable
	{
		...
		/// </summary>
		private List<HtmlHelpTocItem> _mergeLinks = new List<HtmlHelpTocItem>();
		/// <summary>
		/// Internal member storing the read information types
		/// </summary>
        private List<InformationType> _informationTypes = new List<InformationType>();
		/// <summary>
		...
		/// </summary>
        private List<HtmlHelpIndexItem> _indexKLinks = new List<HtmlHelpIndexItem>();
		/// <summary>
		/// Internal arraylist containing the index (alinks)
		/// </summary>
        private List<HtmlHelpIndexItem> _indexALinks = new List<HtmlHelpIndexItem>();
		/// <summary>
		...
		/// </summary>
		private HtmlHelpFileInfo _chmFileInfo = null;
		/// <summary>
			...
			if( File.Exists(chmFile) )
			{
				...
				if(_systemFile.DefaultTopic.Length > 0)
				{
					...
					else 
					{
						...
						if(_toc.Count > 0)
						{
							_systemFile.SetDefaultTopic( ((HtmlHelpTocItem) _toc[0]).Local );
						}
					}
				...
				_chmFileInfo = new HtmlHelpFileInfo(this);
			} 
			else 
			{
						...
						if( (!_dumpRead)||(!_dmpInfo.DumpBinaryIndex) )
						{
							...
							if(storage != null)
							{
								...
								if( fileObject != null)
								{
                                    ...
                                    _indexKLinks = (List<HtmlHelpIndexItem>)_kLinks.IndexList;
									_indexKLinks.Sort();
							...
							if(storage != null)
							{
								...
								if( fileObject != null)
								{
									...
									_indexALinks = (List<HtmlHelpIndexItem>)_aLinks.IndexList;
									_indexALinks.Sort();
		...
		internal List<string> EnumFilesByExtension(string extension)
		{
		...
		internal HtmlHelpTocItem GetTOCItemByLocal(string local)
		{
		...
		/// <param name="local">local to search</param>
		/// <returns>Returns the TOC item</returns>
        private HtmlHelpTocItem GetTOCItemByLocal(IList<HtmlHelpTocItem> arrTOC, string local)
		{
			HtmlHelpTocItem ret = null;
		...
		/// <summary>
		/// Removes a HtmlHelpTocItem from the toc
		/// </summary>
		/// <param name="rem">item to remove</param>
		/// <returns>Returns true if removed</returns>
		internal bool RemoveTOCItem(HtmlHelpTocItem rem)
		{
			if(rem == null)
				return false;
		...
		/// <summary>
		/// Recursively searches a HtmlHelpTocItem and removes it if found
		/// </summary>
		/// <param name="arrTOC">toc level list</param>
		/// <param name="rem">item to remove</param>
		/// <returns>Returns true if removed</returns>
        private bool RemoveTOCItem(IList<HtmlHelpTocItem> arrTOC, HtmlHelpTocItem rem)
		{
			for(int i=0; i<arrTOC.Count;i++)
			{
				HtmlHelpTocItem curItem = arrTOC[i];
		...
		/// <summary>
		/// Gets an list of <see cref="InformationType">InformationType</see> items
		/// </summary>
		...
		/// <summary>
		/// Gets an list of <see cref="Category">Category</see> items
		/// </summary>
        ...
        public List<HtmlHelpTocItem> ParseHHC(string hhcFile)
		{
            List<HtmlHelpTocItem> arrRet = new List<HtmlHelpTocItem>();
			...
			if( fileObject != null)
			{
				...
				if(HHCParser.HasMergeLinks)
				{
					foreach(HtmlHelpTocItem curItem in HHCParser.MergeItems)
					{
		...
		private bool SaveDump(DumpingInfo dmpInfo)
		{
			...
			try
			{
				...
				if(bWriteTOC)
				{
					...
					{
						HtmlHelpTocItem curItem = ((HtmlHelpTocItem)(_toc[nCnt]));
				...
				if(bWriteIdx)
				{
					...
					{
						HtmlHelpIndexItem curItem = _indexALinks[nCnt];
						curItem.Dump( ref writer );
					}
					...
					{
						HtmlHelpIndexItem curItem = _indexKLinks[nCnt];
						curItem.Dump( ref writer );
					}
				}
				...
				{
					InformationType curType = _informationTypes[nCnt];
				...
				{
					Category curCat = _categories[nCnt];
		...
		private bool LoadDump(DumpingInfo dmpInfo)
		{
			...
			try
			{
				...
				if(bFlag)
				{
					...
					{
						HtmlHelpTocItem item = new HtmlHelpTocItem();
				...
				if(bFlag)
				{
					...
					{
						HtmlHelpIndexItem item = new HtmlHelpIndexItem();
						item.ChmFile = this;
						item.ReadDump(ref reader);
		...
		/// </summary>
		internal List<HtmlHelpTocItem> MergLinks
		{
		...
		/// </summary>
		public HtmlHelpFileInfo FileInfo
		{
		...
		/// </summary>
        public List<HtmlHelpTocItem> TOC
		{
			get { return _toc; }
		}
		...
		/// </summary>
        public IList<HtmlHelpIndexItem> IndexKLinks
		{
		...
		/// Gets the internal index read from the chm file.
		/// </summary>
        public IList<HtmlHelpIndexItem> IndexALinks
		{
