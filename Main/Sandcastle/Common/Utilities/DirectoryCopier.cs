...
namespace Sandcastle.Utilities
{
    ...
    /// </summary>
    [Serializable]
    public sealed class DirectoryCopier : HelpObject<DirectoryCopier>
    {
        ...
        public int Copy(string sourceDir, string targetDir, HelpLogger logger)
        {
            _copiedCount = 0;
            ...
            HelpLogger logger)
        {
            ...
            bool isLogging = (logger != null && logger.Logging &&
                logger.Verbosity == HelpLoggerVerbosity.Detailed);
            ...
            for (int i = 0; i < fileCount; i++)
            {
                ...
                if (isLogging)
                {
                    logger.WriteLine(String.Format(@"Copying {0}\{1}", 
                        target.FullName, fi.Name), HelpLoggerLevel.Info);
                }
